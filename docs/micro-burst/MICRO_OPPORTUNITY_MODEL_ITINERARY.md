...
- 2026-09-04 — `70a3ee3`: serialized nested coverage telemetry so the configured pretty logger exposes per-symbol metrics; `677c885`: removed per-symbol `exchange.getServerTime()` calls from shared-plane evaluation by passing one local snapshot timestamp per evaluation cycle, with focused tests passing (34/34).
- 2026-09-04 — `b7abb8f`: aligned fast-state reads with the sampler tick timestamp, preventing millisecond-scale false `invalid_or_future_state` rejections. After a 10-minute LIVE soak from restart cutoff, all 11 symbols persisted 398/402 attempts each (4 initial `no_slow_state`, zero invalid/future states), for 4,378 samples total. Books reached 11/11 healthy, PM2 had zero additional restarts, REST rate-limit events/circuit trips were zero, depth queue depth was zero, and diagnostics reported two startup REST candle fallbacks. Event-loop latency p95 was 692.92 ms. Opportunity coverage and REST-path gates pass for this rollout; evaluation duration remains high at roughly 24–35 s p95 and should be investigated before broader rollout. No J/K/L completion claim is made.
