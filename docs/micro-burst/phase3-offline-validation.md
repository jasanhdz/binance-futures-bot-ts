...
Current directed result: `51/51` tests passing across the offline variant, observer,
...
result above includes those regressions. The repository-wide suite was last run
before that short-write correction; it has not been rerun after the current
follow-up, so this document does not claim a green full validation.
