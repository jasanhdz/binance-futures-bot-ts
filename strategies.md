...
| Impulse Pullback Continuation  |             0 |        — |         — |            — |          — |           — |
...
## Impulse Pullback Continuation (`impulse_pullback_continuation.ts`)

- **Core Idea** — Sigue el patrón clásico impulso-retroceso-continuación combinando lectura de acción del precio con una EMA 50. Detecta zonas clave en diario, confirma cambio de estructura en el timeframe intermedio y dispara la entrada en 5m cuando el precio recupera la EMA tras el retroceso.
- **Fortalezas** — Alinea múltiples timeframes para aumentar la probabilidad: el diario define el contexto, el 1h valida la tendencia y el 5m entrega el gatillo. Stop lógico debajo/encima del swing reciente y uso conservador de objetivos en niveles previos reducen drawdowns.
- **Debilidades** — Requiere paciencia y práctica para identificar zonas válidas; si el retroceso no toca la EMA puede quedarse sin operación. En mercados muy volátiles los retesteos profundos pueden barrer el stop antes de continuar.
- **Qué Mejorar** — Añadir registro de patrones de velas al acercarse a la zona HTF, permitir confirmaciones con volumen decreciente durante el pullback y automatizar targets parciales según ATR.
- **Conclusión** — Estrategia de continuación disciplinada para tendencias limpias; ideal para traders que priorizan evitar pérdidas sobre capturar movimientos extremos.
