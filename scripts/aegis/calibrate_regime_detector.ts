...
function parseArgs(args: string[]): CliOptions {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        else if (arg === '--strong-min-count') options.thresholds = { ...options.thresholds, strongMinCount: Number(next()) };
        else if (arg === '--strong-min-mfemae') options.thresholds = { ...options.thresholds, strongMinMfeMae: Number(next()) };
        else if (arg === '--strong-min-hit8') options.thresholds = { ...options.thresholds, strongMinHit8: Number(next()) };
        else if (arg === '--allow-min-count') options.thresholds = { ...options.thresholds, allowMinCount: Number(next()) };
        else if (arg === '--allow-min-mfemae') options.thresholds = { ...options.thresholds, allowMinMfeMae: Number(next()) };
        else if (arg === '--allow-min-hit8') options.thresholds = { ...options.thresholds, allowMinHit8: Number(next()) };
        else if (arg === '--insufficient-count') options.thresholds = { ...options.thresholds, insufficientCount: Number(next()) };
