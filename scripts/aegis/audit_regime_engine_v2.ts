...
async function main(): Promise<void> {
  ...
  if (report.outputFiles) {
    console.log(
      `Reports written: ${report.outputFiles.markdown}, ${report.outputFiles.json}, ${report.outputFiles.csv}, ${report.outputFiles.recommendations}`,
    );
  }
}
...
function parseArgs(args: string[]): CliOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === '--horizons' && next)
      options.horizons = args[++i]
        .split(',')
        .map((value) => Number(value.trim()))
        .filter(
          (value): value is 15 | 30 | 60 | 120 =>
