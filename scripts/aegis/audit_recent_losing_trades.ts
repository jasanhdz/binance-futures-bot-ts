...
async function main(): Promise<void> {
  ...
  if (report.outputFiles) {
    ...
    if (report.outputFiles.charts.length > 0)
      console.log(`Charts written: ${report.outputFiles.charts.join(', ')}`);
  }
}
...
function parseArgs(args: string[]): CliOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === '--include-control' && next)
      options.includeControl = args[++i].trim().toUpperCase();
