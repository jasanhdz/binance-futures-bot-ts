...
describe('RegimeEngineV2', () => {
  ...
  it('degrades breakout up when adverse wick is high', () => {
    const candles = [
      ...Array.from({ length: 132 }, (_, index) =>
        candle(index, 100, 100.4, 99.7, 100 + Math.sin(index) * 0.08, 100),
      ),
  ...
  it('degrades breakout down when adverse wick is high', () => {
    const candles = [
      ...Array.from({ length: 132 }, (_, index) =>
        candle(index, 100, 100.3, 99.6, 100 + Math.sin(index) * 0.08, 100),
      ),
  ...
  it('classifies MOMENTUM_UP_EARLY without overextension', () => {
    const candles = [
      ...Array.from({ length: 112 }, (_, index) =>
        candle(index, 100, 100.25, 99.75, 100 + Math.sin(index / 3) * 0.05, 100),
      ),
...
function trendingCandles(
  direction: 'UP' | 'DOWN',
  count: number,
  options: { step: number; volume: number; startIndex?: number; startPrice?: number },
...
function shortBreakdownCandles(
  kind: 'CONFIRMED' | 'ABSORBED' | 'SWEEP' | 'WEAK_VOLUME' | 'EXTENDED' | 'RETEST_CONFIRMED',
): RegimeEngineV2InputCandle[] {
