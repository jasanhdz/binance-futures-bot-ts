...
export class RegimeEngineV2 {
    static evaluate(input: RegimeEngineV2EvaluateInput): RegimeEngineV2Decision {
        ...
        const confidence = confidenceFor(technical.regime, scores, transition.risk, marketConfirmationState);
        ...
        return decision({
            ...
            reasons: [...technical.reasons, ...environmentReasons(momentumEnvironment, marketConfirmationState, transition.risk)]
        });
    }
}
...
function calculateIndicators(candles: RegimeEngineV2InputCandle[]): RegimeEngineV2Indicators {
    ...
    const previousBollingerWidth = bollingerWidths.length > 1 ? bollingerWidths[bollingerWidths.length - 2] : undefined;
    ...
    const bodies = candles.slice(-100).map((candle) => candle.close > 0 ? Math.abs(candle.close - candle.open) / candle.close : 0);
    ...
    const breakoutRange = breakoutLevelUp !== undefined && breakoutLevelDown !== undefined ? breakoutLevelUp - breakoutLevelDown : undefined;
    const breakoutCloseBeyondRangePct = breakoutUp && breakoutLevelUp !== undefined && current.close > 0
    ...
    const breakoutFollowThroughScore = breakoutUp || breakoutDown
        ? clamp01(avgDefined([
            breakoutCloseBeyondRangePct !== undefined ? breakoutCloseBeyondRangePct / 0.004 : undefined,
    ...
    return compact({
        ...
        atrPercentile: round(atr !== undefined ? percentileRank(atrSeriesValues.slice(-120), atr) : undefined),
...
function calculateScores(indicators: RegimeEngineV2Indicators): RegimeEngineV2Scores {
    ...
    const chopRisk = clamp01(Math.max(
    ...
    const transitionRisk = clamp01(Math.max(
        exhaustionRisk,
        (indicators.failedBreakoutCount ?? 0) >= 2 ? 0.75 : 0,
        indicators.rangeBreakout !== 'NONE' && (indicators.closeLocation ?? 0.5) > 0.25 && (indicators.closeLocation ?? 0.5) < 0.75 ? 0.55 : 0
    ));
    const momentumQuality = clamp01(avgDefined([
        trendStrength,
        1 - chopRisk,
        clamp01(((indicators.volumeRatio ?? 1) - 0.8) / 0.9),
        indicators.closeLocation !== undefined ? Math.abs(indicators.closeLocation - 0.5) * 2 : undefined,
...
function classifyTechnicalRegime(
...
): { regime: RegimeEngineV2TechnicalRegime; direction: RegimeEngineV2Direction; reasons: string[] } {
    const upStack = (indicators.ema7 ?? 0) > (indicators.ema25 ?? Infinity) && (indicators.ema25 ?? 0) > (indicators.ema99 ?? Infinity);
    const downStack = (indicators.ema7 ?? Infinity) < (indicators.ema25 ?? 0) && (indicators.ema25 ?? Infinity) < (indicators.ema99 ?? 0);
    ...
    if (indicators.rangeBreakout === 'UP' && volumeRatio >= 1.25 && closeLocation >= 0.68 && scores.exhaustionRisk < 0.72 && absDistance <= 0.038 && (indicators.failedBreakoutCount ?? 0) <= 2) {
        ...
        if (breakoutUpProblems.includes('breakout_failed_pressure_high')) return { regime: 'CHOP', direction: 'NONE', reasons: [...breakoutUpProblems, 'breakout_degraded_to_avoid'] };
    ...
    if (scores.chopRisk >= 0.62 && indicators.structure === 'MIXED' && (indicators.bollingerWidthPercentile ?? 0.5) < 0.45) {
        return { regime: 'ACCUMULATION_RANGE', direction: 'NONE', reasons: ['mixed_structure_compressed_range'] };
    }
    if (upStack && (scores.exhaustionRisk >= 0.76 || (distance > 0.035 && (indicators.wickRatio ?? 0) >= 0.38))) {
        return { regime: 'MOMENTUM_UP_EXHAUSTED', direction: 'LONG', reasons: ['up_momentum_overextended_or_upper_wick'] };
    }
    if (downStack && (scores.exhaustionRisk >= 0.76 || (distance < -0.035 && (indicators.wickRatio ?? 0) >= 0.38))) {
        return { regime: 'MOMENTUM_DOWN_EXHAUSTED', direction: 'SHORT', reasons: ['down_momentum_overextended_or_lower_wick'] };
    }
    if (scores.volatilityRisk >= 0.99 && scores.momentumQuality < 0.5 && indicators.rangeBreakout === 'NONE') {
        return { regime: 'HIGH_VOL_RISK', direction: 'NONE', reasons: ['atr_percentile_extreme_without_clean_momentum'] };
    }
    if (upStack && stackAge >= 48 && scores.trendStrength >= 0.48) {
        return { regime: 'MOMENTUM_UP_MATURE', direction: 'LONG', reasons: ['long_ema_stack_mature_trend'] };
    }
    if (downStack && stackAge >= 48 && scores.trendStrength >= 0.48) {
        return { regime: 'MOMENTUM_DOWN_MATURE', direction: 'SHORT', reasons: ['short_ema_stack_mature_trend'] };
    }
    if ((indicators.ema7 ?? 0) > (indicators.ema25 ?? Infinity) && ema25Slope > 0 && scores.momentumQuality >= 0.52 && closeLocation >= 0.58 && absDistance <= 0.03 && scores.chopRisk < 0.68) {
        return { regime: 'MOMENTUM_UP_EARLY', direction: 'LONG', reasons: ['ema7_above_ema25_slope_volume_no_overextension'] };
    }
    if ((indicators.ema7 ?? Infinity) < (indicators.ema25 ?? 0) && ema25Slope < 0 && scores.momentumQuality >= 0.52 && closeLocation <= 0.42 && absDistance <= 0.03 && scores.chopRisk < 0.68) {
        return { regime: 'MOMENTUM_DOWN_EARLY', direction: 'SHORT', reasons: ['ema7_below_ema25_slope_volume_no_overextension'] };
    }
    if (upStack && distance <= 0.006 && distance >= -0.018 && scores.chopRisk < 0.7 && adxSlope >= -1) {
        return { regime: 'TREND_UP_PULLBACK', direction: 'LONG', reasons: ['uptrend_pullback_to_ema25'] };
    }
    if (downStack && distance >= -0.006 && distance <= 0.018 && scores.chopRisk < 0.7 && adxSlope >= -1) {
        return { regime: 'TREND_DOWN_PULLBACK', direction: 'SHORT', reasons: ['downtrend_pullback_to_ema25'] };
    }
    return { regime: 'UNKNOWN', direction: 'NONE', reasons: ['mixed_or_unclassified_context'] };
}

function breakoutProblems(indicators: RegimeEngineV2Indicators, scores: RegimeEngineV2Scores, side: RegimeEngineV2Direction): string[] {
    const reasons: string[] = [];
    if ((indicators.breakoutCloseBeyondRangePct ?? 0) < 0.0012) reasons.push('breakout_close_not_far_enough');
    if ((indicators.breakoutBodyConfirmation ?? 0) < 0.42) reasons.push('breakout_body_too_weak');
    if ((indicators.adverseWickAgainstBreakout ?? 0) > 0.34) reasons.push('breakout_adverse_wick_high');
    if ((indicators.breakoutVolumePersistence ?? 0) < 1.03 && (indicators.volumeRatio ?? 0) < 1.45) reasons.push('breakout_volume_not_persistent');
    if ((indicators.breakoutTooExtendedFromEma25 ?? 0) > 0) reasons.push('breakout_too_extended_from_ema25');
    if ((indicators.failedBreakoutPressure ?? 0) >= 0.72) reasons.push('breakout_failed_pressure_high');
    if (scores.transitionRisk >= 0.78) reasons.push('transition_risk_high');
    if (side === 'LONG' && (indicators.closeLocation ?? 0.5) < 0.7) reasons.push('breakout_close_location_not_strong');
    if (side === 'SHORT' && (indicators.closeLocation ?? 0.5) > 0.3) reasons.push('breakout_close_location_not_strong');
    return reasons;
}
...
function classifyTransition(
    ...
    const risk: RegimeEngineV2TransitionRisk = scores.transitionRisk >= 0.72 ? 'HIGH'
        : scores.transitionRisk >= 0.45 ? 'MODERATE'
            : 'LOW';
    const possibleNextRegime = risk === 'HIGH'
        ? regime.includes('UP') ? 'CHOP_OR_PULLBACK'
            : regime.includes('DOWN') ? 'CHOP_OR_PULLBACK'
                : 'UNKNOWN'
        : undefined;
    return { risk, possibleNextRegime, reasons: reasons.length > 0 ? reasons : ['transition_risk_normal'] };
}

function classifyMarketConfirmation(
    direction: RegimeEngineV2Direction,
    market?: { btc?: { action?: string; score?: number; direction?: string }; eth?: { action?: string; score?: number; direction?: string } }
): RegimeEngineV2MarketConfirmationState {
...
function classifyMomentumEnvironment(input: {
    ...
    if (input.regime === 'CHOP' || input.regime === 'ACCUMULATION_RANGE' || input.regime === 'HIGH_VOL_RISK') return 'AVOID_MOMENTUM';
    if (input.regime.includes('EXHAUSTED')) return input.transitionRisk === 'HIGH' ? 'AVOID_MOMENTUM' : watchFor(input.direction);
    if (input.marketConfirmationState === 'CONTRADICT') return input.transitionRisk === 'HIGH' ? 'AVOID_MOMENTUM' : watchFor(input.direction);
    if (input.transitionRisk === 'HIGH') return input.regime.includes('EARLY') ? watchFor(input.direction) : 'AVOID_MOMENTUM';
    if (input.regime.includes('MATURE') || input.regime.includes('PULLBACK')) return watchFor(input.direction);
    if (input.regime.includes('BREAKOUT') && input.regime.includes('EARLY')) return allowFor(input.direction);
    if (input.regime.includes('EARLY') && input.scores.momentumQuality >= 0.52) return allowFor(input.direction);
...
function confidenceFor(regime: RegimeEngineV2TechnicalRegime, scores: RegimeEngineV2Scores, transitionRisk: RegimeEngineV2TransitionRisk, market: RegimeEngineV2MarketConfirmationState): number {
    if (regime === 'UNKNOWN') return 0.25;
    const base = regime === 'CHOP' ? scores.chopRisk
        : regime === 'HIGH_VOL_RISK' ? scores.volatilityRisk
            : avgDefined([scores.trendStrength, scores.momentumQuality, 1 - scores.chopRisk, 1 - scores.exhaustionRisk]) ?? 0.4;
    const transitionPenalty = transitionRisk === 'HIGH' ? 0.15 : transitionRisk === 'MODERATE' ? 0.06 : 0;
    const marketPenalty = market === 'CONTRADICT' ? 0.12 : 0;
    return clamp01(base - transitionPenalty - marketPenalty);
}

function environmentReasons(environment: RegimeEngineV2MomentumEnvironment, market: RegimeEngineV2MarketConfirmationState, transitionRisk: RegimeEngineV2TransitionRisk): string[] {
...
function validCandle(candle: RegimeEngineV2InputCandle): boolean {
    return [candle.open, candle.high, candle.low, candle.close, candle.volume].every((value) => Number.isFinite(value));
}
...
function emaStackAge(ema7: number[], ema25: number[], ema99: number[], direction: RegimeEngineV2Direction): number | undefined {
...
function marketStructure(candles: RegimeEngineV2InputCandle[]): RegimeEngineV2Indicators['structure'] {
    ...
    if (firstHigh === undefined || firstLow === undefined || secondHigh === undefined || secondLow === undefined) return 'UNKNOWN';
...
function volumeTrend(volumes: number[]): number | undefined {
    ...
    return recent !== undefined && previous !== undefined && previous > 0 ? (recent - previous) / previous : undefined;
}
...
function seriesSlopePct(values: number[], lookback: number): number | undefined {
    ...
    return current !== undefined && prior !== undefined && prior !== 0 ? (current - prior) / prior : undefined;
}
