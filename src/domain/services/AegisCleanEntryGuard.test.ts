...
describe('AegisCleanEntryGuard', () => {
  ...
  it('Python model ok + rule gate insufficient_data + EventRisk false permite ALLOW_CLEAN', () => {
    ...
    expect(
      decision.metadata.clean_entry_rule_gate_insufficient_context_ignored_due_to_model_ok,
    ).toBe(true);
  });
  ...
  it('extreme momentum candidate se marca solo en shadow', () => {
    const shadow = evaluateAegisCleanEntryGuard(
      input({
        mode: 'SHADOW',
        turboScore: 0.98,
        votes: { long: 3, short: 0, neutral: 0 },
    ...
    const enforce = evaluateAegisCleanEntryGuard(
      input({
        mode: 'ENFORCE',
        turboScore: 0.98,
        votes: { long: 3, short: 0, neutral: 0 },
