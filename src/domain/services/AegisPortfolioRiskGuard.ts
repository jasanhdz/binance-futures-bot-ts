...
export class AegisPortfolioRiskGuard {
    static evaluate(input: AegisPortfolioRiskInput): AegisPortfolioRiskDecision {
        ...
        const sameDirectionPositions = input.side === 'LONG'
            ? input.currentLongPositions
            : input.currentShortPositions;
        const equityBase = finiteNumber(input.equityTotal) && input.equityTotal > 0
            ? input.equityTotal
        ...
        if (limits.maxOpenPositions !== undefined && input.currentOpenPositions >= limits.maxOpenPositions) {
            return { allowed: false, reason: 'max_open_positions_reached', metadata };
        }

        if (limits.maxSameDirectionPositions !== undefined && sameDirectionPositions >= limits.maxSameDirectionPositions) {
