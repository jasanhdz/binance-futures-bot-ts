...
function hasCriticalMissingData(input: AegisCleanEntryGuardInput): boolean {
  return (
    !normalized(input.decisionBrain) ||
...
function isExtremeMomentumCandidate(input: AegisCleanEntryGuardInput): boolean {
  ...
  if (!finiteNumber(input.turboScore) || input.turboScore < input.exception.minTurboScore)
    return false;
  if (!finiteNumber(input.tailRiskScore) || input.tailRiskScore > input.exception.maxTailRiskScore)
    return false;
  return true;
}

export function evaluateAegisCleanEntryGuard(
  input: AegisCleanEntryGuardInput,
  ...
  const entryQualityModelRecommendation = normalized(
    input.entryQualityModelRecommendation ?? input.entryQualityRecommendation,
  );
  const entryQualityRecommendation = entryQualityModelRecommendation;
  const entryQualityModelFeatureStatus = normalized(
    input.entryQualityModelFeatureStatus ?? input.featureStatus,
  );
  const entryQualityRuleGateReason = normalized(
    input.entryQualityRuleGateReason ?? input.entryQualityGateReason,
  );
  const entryQualityRuleGateDecision = normalized(
    input.entryQualityRuleGateDecision ?? input.entryQualityGateAction,
  );
  ...
  const minFeatureParityPct = finiteNumber(input.minFeatureParityPct)
    ? input.minFeatureParityPct
    : 95;
  ...
  const modelPresent =
    input.entryQualityModelPresent !== undefined
      ? input.entryQualityModelPresent === true
      : inferredModelPresent;
  const modelFeaturesOk =
    modelPresent &&
  ...
  const ignoreRuleInsufficientWhenModelOk =
    input.ignoreRuleGateInsufficientDataWhenModelOk !== false;
  ...
  if (useModelSourceOfTruth && !modelFeaturesOk) {
  ...
  if (input.dirtyConditions.blockWhenEventRiskWouldBlock && input.eventRiskWouldBlock === true) {
  ...
  const clean =
    ...
    (!useModelSourceOfTruth || modelFeaturesOk) &&
    (!input.cleanConditions.requireEntryQualityAllow ||
      isEntryQualityAllow(entryQualityRecommendation)) &&
    (!input.cleanConditions.requireNoInsufficientData ||
      !ruleGateInsufficientData ||
      (useModelSourceOfTruth && ignoreRuleInsufficientWhenModelOk && modelFeaturesOk)) &&
    (!input.cleanConditions.requireEventRiskWouldBlockFalse ||
      input.eventRiskWouldBlock === false) &&
    finiteNumber(input.tailRiskScore) &&
    input.tailRiskScore <= input.cleanConditions.maxTailRiskScore;
  ...
  function buildOutput(
    ...
    const metadata = {
      ...
      entryQualityModelMissingFeaturesCount:
        source.entryQualityModelMissingFeaturesCount ?? source.missingFeaturesCount,
      ...
      clean_entry_rule_gate_insufficient_context_ignored_due_to_model_ok:
        ...
        ignoreRuleInsufficientWhenModelOk &&
        modelFeaturesOk
          ? true
          : undefined,
    };
