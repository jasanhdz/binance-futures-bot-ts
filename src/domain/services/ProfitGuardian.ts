...
export const DEFAULT_GUARDIAN_CONFIG: GuardianConfig = {
  ...
  useAtrTrailing: true,
  atrMultiplier: 2.0,
  minimumProtectedRoeRatio: 0.7,
  minimumProtectedRoeAbs: 0.2,
};
...
export function evaluateGuardianAction(
  ...
  if (currentPeakRoe >= activationRoe) {
    ...
    if (config.useAtrTrailing && config.atrMultiplier && atrValue && atrValue > 0) {
      ...
      const atrTriggerPrice =
        positionSide === 'SHORT' ? peakPrice + trailingDistance : peakPrice - trailingDistance;

      // Calculate ROE at ATR-based trigger price
      const roeAtAtrTrigger =
        positionSide === 'SHORT'
      ...
      const minimumProtectedRoe = Math.max(
        currentPeakRoe * (config.minimumProtectedRoeRatio ?? 0.7),
        config.minimumProtectedRoeAbs ?? 0.2,
      );
    ...
    const shouldClose =
      positionSide === 'SHORT' ? currentPrice >= triggerPrice : currentPrice <= triggerPrice;
  ...
  const isBeTriggered =
    (peakRoe !== undefined && peakRoe >= config.beTriggerRoe) || roe >= config.beTriggerRoe;
  ...
  const isInProfit =
    positionSide === 'SHORT' ? currentPrice < entryPrice : currentPrice > entryPrice;
  ...
  if (isInProfit && (isBeTriggered || slIsAtBe) && !config.trailingActivationRoe) {
    ...
    const wouldExecuteNow =
      positionSide === 'SHORT' ? currentPrice >= trailingSlPrice : currentPrice <= trailingSlPrice;
