...
describe('AegisProbeMode', () => {
    ...
    it('denies when TailRisk is above 0.30', () => {
        expect(AegisProbeMode.evaluate(input({ tailRiskScore: 0.31 })).reason).toBe('probe_tail_risk_too_high');
    });

    it('denies when turbo score is below 0.90', () => {
        expect(AegisProbeMode.evaluate(input({ turboScore: 0.899 })).reason).toBe('probe_turbo_score_too_low');
    });

    it('denies DecisionBrain WAIT_CONFIRMATION', () => {
        expect(AegisProbeMode.evaluate(input({ decisionBrain: 'WAIT_CONFIRMATION' })).reason).toBe('probe_decision_brain_not_enter_now');
    });

    it('denies EntryQuality BLOCK_SHADOW', () => {
        expect(AegisProbeMode.evaluate(input({ entryQualityRecommendation: 'BLOCK_SHADOW' })).reason).toBe('probe_entry_quality_block_shadow');
    });

    it('denies RISK_OFF', () => {
        expect(AegisProbeMode.evaluate(input({ eventRiskMode: 'RISK_OFF' })).reason).toBe('probe_event_risk_risk_off');
    });

    it('denies MANUAL_ONLY', () => {
        expect(AegisProbeMode.evaluate(input({ eventRiskMode: 'MANUAL_ONLY' })).reason).toBe('probe_event_risk_manual_only');
    });

    it('respects min_minutes_between_probe_entries', () => {
        const nowMs = Date.parse('2026-05-20T12:00:00Z');
        expect(AegisProbeMode.evaluate(input({ nowMs, lastProbeAt: nowMs - 30 * 60 * 1000 })).reason)
            .toBe('probe_min_minutes_between_entries');
    });

    it('respects max_probe_entries_per_hour', () => {
        const nowMs = Date.parse('2026-05-20T12:00:00Z');
        expect(AegisProbeMode.evaluate(input({ nowMs, probeEntryTimestamps: [nowMs - 45 * 60 * 1000] })).reason)
            .toBe('probe_entries_per_hour_exceeded');
    });

    it('respects max_open_probe_positions', () => {
        expect(AegisProbeMode.evaluate(input({ openProbePositions: 1 })).reason).toBe('probe_open_probe_positions_exceeded');
    });

    it('respects max_total_open_positions_when_probe', () => {
        expect(AegisProbeMode.evaluate(input({ totalOpenPositions: 2 })).reason).toBe('probe_total_open_positions_exceeded');
    });

    it('does not allow after configured consecutive losses', () => {
        expect(AegisProbeMode.evaluate(input({ consecutiveLosses: 2 })).reason).toBe('probe_consecutive_losses');
    });

    it('does not allow after recent stop loss', () => {
        const nowMs = Date.parse('2026-05-20T12:00:00Z');
        expect(AegisProbeMode.evaluate(input({ nowMs, lastStopLossAt: nowMs - 10 * 60 * 1000 })).reason)
            .toBe('probe_recent_stop_loss');
    });

    it('denies non-EventRisk Clean Entry block reasons', () => {
        expect(AegisProbeMode.evaluate(input({ cleanEntryReasons: ['clean_entry_tail_risk_high'] })).reason)
