...
function hasRegime(config: AegisRegimeGuardConfig, list: 'allowWhen' | 'blockWhen', regime: AegisRegimeLabel): boolean {
    return Array.isArray(config[list]) && config[list].includes(regime);
}

function baseMetadata(input: AegisRegimeGuardInput, source: AegisRegimeSource): Record<string, unknown> {
    return {
        symbol: input.symbol,
        side: input.side,
        source,
...
function unknownDecision(input: AegisRegimeGuardInput, reason: AegisRegimeGuardReason): AegisRegimeDecision {
    return decision(input, 'UNKNOWN', 0.30, reason, 'HYBRID_HEURISTIC', hasRegime(input.config, 'blockWhen', 'UNKNOWN'));
}
...
export class AegisRegimeGuard {
    static evaluate(input: AegisRegimeGuardInput): AegisRegimeDecision {
        ...
        const eventRiskMode = String(input.eventRiskMode || '').trim().toUpperCase();
        ...
        return unknownDecision({ ...input, config }, hasRegime(config, 'blockWhen', 'UNKNOWN')
            ? 'regime_unknown_block'
            : 'regime_low_confidence');
    }
}
