...
function context(overrides: Partial<AegisEntryContext> = {}): AegisEntryContext {
    const candles = Array.from({ length: 25 }, (_, index) => {
        ...
        return { open, high: open + 0.02, low: open - 0.005, close: open + 0.015, volume: index === 24 ? 200 : 100 };
    });
    return {
        symbol: 'XRPUSDT',
        side: 'LONG',
        ...
        signal: { symbol: 'XRPUSDT', action: 'LONG', confidence: 0.94, source: 'AEGIS_TURBO', longProb: 0.94, shortProb: 0.03, neutralProb: 0.03 },
        gate: { allowed: true, side: 'LONG', reason: 'ok', leverage: 20, positionFraction: 0.01, stopRoe: -0.15, takeProfitRoe: 0.25, trailingActivationRoe: 0.15, trailingCallbackRoe: 0.08, turboScore: 0.94, votes: { long: 3 } },
        entryQuality: {
...
describe('RegimeContextGuardAdapter', () => {
    ...
    it('ENFORCE sigue siendo informativo y no bloquea globalmente', () => {
        const result = RegimeContextGuardAdapter.evaluate(context(), { enabled: true, mode: 'ENFORCE' });
