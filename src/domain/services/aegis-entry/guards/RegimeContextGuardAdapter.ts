...
import { AegisRegimeGuard, AegisRegimeLabel } from '../../AegisRegimeGuard';
...
export class RegimeContextGuardAdapter {
    static evaluate(context: AegisEntryContext, policy: AegisEntryGuardPolicy): RegimeContextGuardAdapterResult {
        ...
        const hasEnoughData = indicators.volumeRatio !== undefined
        ...
        const regimeContext = buildRegimeContext(decision.regime, decision.confidence, indicators, config.thresholds, [
            decision.reason
        ]);
...
function buildRegimeContext(
    ...
    const trendDirection = label === 'MOMENTUM_UP' || label === 'TREND_UP' || label === 'BREAKOUT_UP'
        ? 'UP'
        : label === 'MOMENTUM_DOWN' || label === 'TREND_DOWN' || label === 'BREAKOUT_DOWN'
            ? 'DOWN'
            : inferTrendDirection(indicators);
    const chopRisk = indicators.choppiness !== undefined
        ? clamp01(indicators.choppiness / 100)
        : label === 'CHOP' ? 0.85 : 0.3;
    const exhaustionRisk = label === 'EXHAUSTION'
        ? 0.85
        : (indicators.atrPercentile !== undefined && indicators.atrPercentile >= 0.9 ? indicators.atrPercentile : 0.2);
    const adxOk = indicators.adx === undefined || indicators.adx >= thresholds.minAdxForMomentum;
    const chopOk = indicators.choppiness === undefined || indicators.choppiness <= thresholds.maxChoppinessForMomentum;
    const volumeOk = indicators.volumeRatio === undefined || indicators.volumeRatio >= thresholds.minVolumeRatioForMomentum;
    const exhaustionOk = exhaustionRisk <= thresholds.maxExhaustionScore;
    const longLabelAllowed = label === 'MOMENTUM_UP' || label === 'TREND_UP' || label === 'BREAKOUT_UP';
    const shortLabelAllowed = label === 'MOMENTUM_DOWN' || label === 'TREND_DOWN' || label === 'BREAKOUT_DOWN';
    ...
    return {
        ...
        volatilityState: indicators.atrPercentile === undefined
            ? 'UNKNOWN'
            : indicators.atrPercentile >= thresholds.maxAtrPercentileForAggressive ? 'HIGH' : 'NORMAL',
        volumeState: indicators.volumeRatio === undefined
            ? 'UNKNOWN'
            : indicators.volumeRatio >= thresholds.minVolumeRatioForMomentum ? 'HIGH' : 'NORMAL',
        reasons,
        indicators
    };
}

function buildIndicators(context: AegisEntryContext, volumeWindow: number): AegisRegimeContext['indicators'] {
    const candles = context.entryQuality.ruleGate.recentCandles ?? [];
    const latest = candles[candles.length - 1];
    const previous = candles[candles.length - 2];
    const volumeHistory = candles.slice(Math.max(0, candles.length - volumeWindow - 1), -1)
        .map((candle) => candle.volume)
        .filter((value): value is number => isFiniteNumber(value));
    const avgVolume = average(volumeHistory);
    const volumeRatio = latest?.volume !== undefined && avgVolume && avgVolume > 0
        ? latest.volume / avgVolume
        : undefined;
    const emaFast = context.entryQuality.ruleGate.emaFast;
    const emaFastSlope = previous && latest && previous.close > 0 ? (latest.close - previous.close) / previous.close : undefined;
...
function inferTrendDirection(indicators: AegisRegimeContext['indicators']): AegisRegimeContext['trendDirection'] {
    if (indicators.emaFast !== undefined && indicators.emaMid !== undefined && indicators.emaSlow !== undefined) {
        if (indicators.emaFast > indicators.emaMid && indicators.emaMid > indicators.emaSlow) return 'UP';
        if (indicators.emaFast < indicators.emaMid && indicators.emaMid < indicators.emaSlow) return 'DOWN';
    }
...
function calculateChoppiness(candles: Array<{ high: number; low: number; close: number }>): number | undefined {
...
function calculateSimpleAdx(candles: Array<{ high: number; low: number; close: number }>): number | undefined {
