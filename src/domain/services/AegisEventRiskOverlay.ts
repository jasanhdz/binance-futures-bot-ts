  ...
  static evaluate(input: AegisEventRiskOverlayInput): AegisEventRiskOverlayDecision {
    ...
    if (mode === 'MANUAL_ONLY') {
      if (input.config?.manualOnly?.blockNewEntries === true) {
        return shadowOrBlock(input, mode, 'SHADOW_MANUAL_ONLY', 'manual_only_requires_approval', {
    ...
    const qualityOk =
      finiteNumber(input.entryQualityScore) && input.entryQualityScore >= minQualityScore;
