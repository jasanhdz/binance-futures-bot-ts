...
import { AegisEntryDecisionResult, AegisEntryGuardResult } from './aegis-entry/AegisEntryDecisionTypes';
...
function guardByName(result: AegisEntryDecisionResult, name: string): AegisEntryGuardResult | undefined {
...
function guardWarns(result: AegisEntryDecisionResult, name: string): boolean {
    ...
    return guard?.wouldBlock === true || guard?.decision === 'DENY' || guard?.decision === 'SHADOW_DENY';
}
...
function hasValidRegimeData(result: AegisEntryDecisionResult): boolean {
    ...
    if (!context || context.trendDirection === 'UNKNOWN' || context.volatilityState === 'UNKNOWN') return false;
    const finiteIndicators = Object.values(context.indicators ?? {})
...
export function evaluateAegisEntrySafetyConsensus(input: {
    ...
    const mode: SafetyMode = config.mode === 'ENFORCE'
        ? 'ENFORCE'
        : config.mode === 'OFF' ? 'OFF' : 'SHADOW';
    const criticalLongVetoMode: SafetyMode = config.criticalLongVetoMode === 'ENFORCE'
        ? 'ENFORCE'
        : config.criticalLongVetoMode === 'SHADOW' ? 'SHADOW' : 'OFF';
    ...
    const derivedWarnings = ['entry_quality', 'event_risk', 'decision_brain', 'clean_entry']
        .filter((name) => guardWarns(input.entryDecision, name));
    ...
    const criticalLongVetoWouldBlock = criticalLongRisk
        && regimeRisk
    ...
    const criticalLongVetoEnforced = enabled
        && criticalLongVetoMode === 'ENFORCE'
        && criticalLongVetoWouldBlock;
    const enforced = consensusEnforced || criticalLongVetoEnforced;
    const wouldBlock = enabled && (
