import {
  AegisDecisionBrainBlock,
  AegisEntryQualityModelBlock,
  AegisEventRiskAutoBlock,
} from './AegisStrategy';
...
function normalizeEventRiskMode(mode?: string): EventRiskMode {
  const normalized = String(mode || 'NORMAL')
    .trim()
    .toUpperCase();
...
function normalizedDecision(input?: AegisDecisionBrainBlock | null): string {
  return String(input?.decision || 'UNKNOWN')
    .trim()
    .toUpperCase();
}

function normalizedRecommendation(input?: AegisEntryQualityModelBlock): string {
  return String(input?.recommendation || 'UNKNOWN')
    .trim()
    .toUpperCase();
}

function extractTailRisk(input: AegisDecisionEnforcementInput): number | undefined {
  if (finiteNumber(input.entryQualityModel?.tail_risk_score))
    return input.entryQualityModel.tail_risk_score;
  return undefined;
}
...
function isEntryQualityShadowBlock(input: AegisDecisionEnforcementInput): boolean {
  return (
    normalizedRecommendation(input.entryQualityModel) === 'BLOCK_SHADOW' ||
...
function isEntryQualityAllow(input: AegisDecisionEnforcementInput): boolean {
  return (
    normalizedRecommendation(input.entryQualityModel) === 'ALLOW_SHADOW' ||
...
function setupGrade(input: AegisDecisionEnforcementInput): AegisDecisionSetupGrade {
  ...
  const baseClean =
    dbEnterNow && eqAllow && !isEntryQualityShadowBlock(input) && tailAvailable && turboAvailable;

  if (!baseClean) return 'WEAK';

  const isA = tailRisk <= 0.5 && turboScore >= 0.8;
  if (!isA) return 'WEAK';

  const btcAction = contextAction(input.eventRiskAuto?.btc_context);
  const ethAction = contextAction(input.eventRiskAuto?.eth_context);
  const btcEthDoesNotContradict =
    !(input.isAltSymbol === true && input.side === 'LONG') ||
    (!contextContradictsSide(btcAction, input.side) &&
      !contextContradictsSide(ethAction, input.side));
  const isAPlus =
    tailRisk <= 0.35 &&
    turboScore >= 0.9 &&
...
export class AegisDecisionEnforcement {
  static evaluate(input: AegisDecisionEnforcementInput): AegisDecisionEnforcementDecision {
    ...
    if (input.config.block_all_entry_quality_shadow_block && isEntryQualityShadowBlock(input)) {
    ...
    if (
      ...
      isEntryQualityShadowBlock(input) &&
      input.config.block_entry_quality_shadow_block_when_event_risk.event_modes.includes(
        eventRiskMode,
      )
    ) {
    ...
    if (input.config.block_caution_would_block_unless_a_plus && eventRiskMode === 'CAUTION') {
