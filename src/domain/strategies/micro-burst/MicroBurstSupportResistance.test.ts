...
function makeCandles(prices: number[]): Candle[] {
  return prices.map((p, i) => makeCandle({
    index: i,
    open: p,
    high: p + 0.5,
    low: p - 0.5,
...
describe('MicroBurstSupportResistance', () => {
  ...
  it('detects support and resistance from swing points', () => {
    const prices = [
      ...
      100, 101, 102, 103, 104, 103, 102, 101, 100, 101,
    ];
  ...
  it('classifies near_support when price is close to support', () => {
    const prices = [
      100.5, 100.4, 100.3, 100.2, 100.1, 100.0, 99.9, 99.8, 99.9, 100.0,
      100.1, 100.0, 99.9, 99.8, 99.9, 100.0, 100.05, 100.03, 100.04, 100.05,
    ];
    const candles = makeCandles(prices);
  ...
  it('computes corridor width', () => {
    const candles = makeCandles([
      98, 99, 100, 101, 102, 101, 100, 99, 98, 99,
      100, 101, 102, 101, 100, 99, 98, 99, 100, 100.5,
    ]);
