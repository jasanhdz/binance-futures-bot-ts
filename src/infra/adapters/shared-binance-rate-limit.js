...
const DEFAULT_DB = (0, node_path_1.join)(
  (0, node_os_1.tmpdir)(),
  'trading_system-binance-shared-rate-limit.sqlite3',
);
...
class SharedBinanceRateLimiter {
  constructor(
    ...
    try {
      ...
      this.runWithBusyRetry(() => this.db.pragma('journal_mode = WAL'));
  ...
  noteRateLimit(until, status) {
    ...
    const transaction = this.db.transaction(() => {
      const state = this.db
        .prepare('SELECT cooldown_until FROM binance_rate_limit_state WHERE id = 1')
        .get();
  ...
  getMetrics() {
    ...
    const tokens = Math.min(
      CAPACITY,
      state.tokens + Math.max(0, Date.now() - state.updated_at) * REFILL_PER_MS,
    );
    return {
      ...this.metrics,
  ...
  tryAcquire(weight, endpoint, priority) {
    ...
    const transaction = this.db.transaction(() => {
      ...
      const tokens = Math.min(
        CAPACITY,
        state.tokens + Math.max(0, now - state.updated_at) * REFILL_PER_MS,
      );
      ...
      this.db
        .prepare('UPDATE binance_rate_limit_state SET tokens = ?, updated_at = ? WHERE id = 1')
        .run(tokens, now);
    }).immediate;
    this.runWithBusyRetry(transaction);
    return waitMs;
  ...
  runWithBusyRetry(transaction) {
    for (let attempt = 0; attempt <= SQLITE_BUSY_RETRIES; attempt += 1) {
      try {
        ...
        if (attempt === SQLITE_BUSY_RETRIES) throw error;
        const jitter =
          SQLITE_BUSY_JITTER_MIN_MS +
          Math.floor(Math.random() * (SQLITE_BUSY_JITTER_MAX_MS - SQLITE_BUSY_JITTER_MIN_MS + 1));
