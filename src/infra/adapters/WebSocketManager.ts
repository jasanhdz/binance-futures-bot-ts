...
export class WebSocketManager {
  ...
  private activeSubscriptions: Record<
    string,
    { type: 'candle' | 'user' | 'aggTrade' | 'depth' | 'depthDiff'; params: any }
  > = {};
  ...
  public connectDepthDiff(
    ...
    const key = `depthDiff:${symbol}:${speed}`;
    this.activeSubscriptions[key] = { type: 'depthDiff', params: { symbol, speed, callback } };
    this.subscribeDepthDiff(key, symbol, speed, callback);
    return () => {
      this.activeCleanups[key]?.();
      delete this.activeCleanups[key];
      delete this.activeSubscriptions[key];
    };
  }

  private subscribePartialDepth(
    key: string,
    symbol: string,
    levels: number,
    speed: '100ms' | '250ms' | '500ms',
    ...
    try {
      const clean = this.client.ws.futuresPartialDepth(
        { symbol, level: levels, updateSpeed: speed } as any,
        (depth: any) => {
  ...
  private subscribeDepthDiff(
    key: string,
    symbol: string,
    speed: '100ms' | '250ms' | '500ms',
    callback: (depth: BinanceDepthDiffEvent) => void,
  ) {
    this.logger.info('WS Connecting USD-M Diff Depth', { symbol, speed });
    try {
      const clean = (this.client.ws.futuresDepth as any)(
        symbol,
        (depth: any) => {
          callback({
            U: depth.U ?? depth.firstUpdateId,
            u: depth.u ?? depth.finalUpdateId,
            pu: depth.pu,
            bids: (depth.b ?? depth.bidDepth ?? []).map((level: any) => [
              String(level[0] ?? level.price),
              String(level[1] ?? level.quantity),
            ]),
            asks: (depth.a ?? depth.askDepth ?? []).map((level: any) => [
              String(level[0] ?? level.price),
              String(level[1] ?? level.quantity),
            ]),
            E: depth.E ?? depth.eventTime,
            T: depth.T ?? depth.transactionTime,
      ...
      this.logger.error('WS USD-M Diff Depth Connection Failed', { key, error: String(e) });
    }
  }

  private subscribeAggTrades(
    key: string,
    symbol: string,
    callback: (trade: { isBuyerMaker: boolean; quantity: string; price: string }) => void,
  ) {
  ...
  private async reconnectAll() {
    ...
    this.logger.info('🔄 Re-subscribing to channels...', {
      count: Object.keys(this.activeSubscriptions).length,
    });
    ...
    for (const [key, sub] of Object.entries(this.activeSubscriptions)) {
      ...
      } else if (sub.type === 'depthDiff') {
        this.subscribeDepthDiff(key, sub.params.symbol, sub.params.speed, sub.params.callback);
      }
    }
