...
export class NinjaConfigManager {
  ...
  getActiveSymbols(): string[] {
    ...
    const QUALITY_THRESHOLD = 0.4; // Lowered from 0.55 to prevent filtering active champions
    ...
    console.log(
      `[NinjaConfig] Active symbols: ${activeSymbols.length}/${allSymbols.length} (threshold: ${QUALITY_THRESHOLD * 100}%)`,
    );
  ...
  getGuardianConfig(
    ...
    return {
      beTriggerRoe: mergedConfig.be_roe ?? 0.1,
      ...
      trailingCallbackRoe: mergedConfig.trailing_callback_roe ?? 0.3,
  ...
  getAegisPortfolioRiskConfig(): Required<AegisPortfolioRiskYamlConfig> {
    ...
    return {
      ...
      max_same_direction_positions: Math.max(
        0,
        Math.floor(this.finiteNumber(raw.max_same_direction_positions, 0)),
      ),
  ...
  getAegisShortGateConfig(): Required<AegisShortGateYamlConfig> {
    ...
    return {
      ...
      position_fraction_multiplier: Math.max(
        0,
        this.finiteNumber(raw.position_fraction_multiplier, 1),
      ),
  ...
  getAegisEventRiskConfig(): AegisEventRiskRuntimeConfig {
    ...
    return {
      ...
      risk_off: {
        ...
        max_tail_risk_score: Math.max(
          0,
          this.finiteNumber(raw.risk_off?.max_tail_risk_score, 0.35),
        ),
  ...
  getAegisRegimeGuardConfig(): AegisRegimeGuardConfig {
    ...
    return {
      ...
      allowWhen: this.normalizeRegimeList(
        raw.allow_when,
        DEFAULT_AEGIS_REGIME_GUARD_CONFIG.allowWhen,
      ),
      blockWhen: this.normalizeRegimeList(
        raw.block_when,
        DEFAULT_AEGIS_REGIME_GUARD_CONFIG.blockWhen,
      ),
      minConfidence: Math.max(
        0,
        this.finiteNumber(raw.min_confidence, DEFAULT_AEGIS_REGIME_GUARD_CONFIG.minConfidence),
      ),
      maxSnapshotAgeSeconds: Math.max(
        0,
        this.finiteNumber(
          raw.max_snapshot_age_seconds,
          DEFAULT_AEGIS_REGIME_GUARD_CONFIG.maxSnapshotAgeSeconds,
        ),
      ),
      ...
      highTailRiskThreshold: Math.max(
        0,
        this.finiteNumber(
          raw.high_tail_risk_threshold,
          DEFAULT_AEGIS_REGIME_GUARD_CONFIG.highTailRiskThreshold,
        ),
      ),
      telemetry: {
  ...
  getAegisRegimeContextConfig(): AegisRegimeContextRuntimeConfig {
    ...
    return {
      ...
      timeframe:
        typeof raw.timeframe === 'string' && raw.timeframe.trim() ? raw.timeframe.trim() : '5m',
      indicators: {
        ...
        bollingerWindow: Math.max(
          1,
          Math.floor(this.finiteNumber(raw.indicators?.bollinger_window, 20)),
        ),
        adxWindow: Math.max(1, Math.floor(this.finiteNumber(raw.indicators?.adx_window, 14))),
        choppinessWindow: Math.max(
          1,
          Math.floor(this.finiteNumber(raw.indicators?.choppiness_window, 14)),
        ),
      },
      thresholds: {
        maxChoppinessForMomentum: Math.max(
          0,
          this.finiteNumber(raw.thresholds?.max_choppiness_for_momentum, 55),
        ),
        minAdxForMomentum: Math.max(0, this.finiteNumber(raw.thresholds?.min_adx_for_momentum, 18)),
        minVolumeRatioForMomentum: Math.max(
          0,
          this.finiteNumber(raw.thresholds?.min_volume_ratio_for_momentum, 1.3),
        ),
        maxAtrPercentileForAggressive: Math.max(
          0,
          this.finiteNumber(raw.thresholds?.max_atr_percentile_for_aggressive, 0.8),
        ),
        maxExhaustionScore: Math.max(
          0,
          this.finiteNumber(raw.thresholds?.max_exhaustion_score, 0.6),
        ),
      },
    };
  }
  ...
  getAegisMomentumRideConfig(): AegisMomentumRideRuntimeConfig {
    ...
    return {
      ...
      symbols,
      protection: {
        hardStopRoe: Math.min(-0.0001, this.finiteNumber(raw.protection?.hard_stop_roe, -0.4)),
        takeProfitRoe: Math.max(0.0001, this.finiteNumber(raw.protection?.take_profit_roe, 0.5)),
        breakEvenRoe: Math.max(0, this.finiteNumber(raw.protection?.break_even_roe, 0.08)),
        trailingActivationRoe: Math.max(
          0,
          this.finiteNumber(raw.protection?.trailing_activation_roe, 0.15),
        ),
        trailingCallbackRoe: Math.max(
          0,
          this.finiteNumber(raw.protection?.trailing_callback_roe, 0.08),
        ),
        maxHoldMs: Math.max(
          60_000,
          Math.floor(this.finiteNumber(raw.protection?.max_hold_ms, 28_800_000)),
        ),
      },
      safetyCaps: {
        maxLeverage,
        maxPositionFraction,
        maxOpenMomentumPositions: Math.max(
          1,
          Math.floor(this.finiteNumber(safety.max_open_momentum_positions, 1)),
        ),
        maxTotalOpenPositionsWhenMomentum: Math.max(
          1,
          Math.floor(this.finiteNumber(safety.max_total_open_positions_when_momentum, 2)),
        ),
        maxMomentumTradesPerDay: Math.max(
          1,
          Math.floor(this.finiteNumber(safety.max_momentum_trades_per_day, 3)),
        ),
        maxConsecutiveMomentumLosses: Math.max(
          1,
          Math.floor(this.finiteNumber(safety.max_consecutive_momentum_losses, 2)),
        ),
        cooldownAfterLossMinutes: Math.max(
          0,
          this.finiteNumber(safety.cooldown_after_loss_minutes, 60),
        ),
        disableSymbolAfterStopLossMinutes: Math.max(
          0,
          this.finiteNumber(safety.disable_symbol_after_stop_loss_minutes, 120),
        ),
        maxLiquidityStress: Math.max(0, this.finiteNumber(safety.max_liquidity_stress, 0.7)),
        dailyLossStopPct: Math.max(0, this.finiteNumber(safety.daily_loss_stop_pct, 0.9)),
  ...
  getAegisProbeModeConfig(): AegisProbeModeRuntimeConfig {
    ...
    return {
      ...
      apply_when_event_risk: Array.isArray(raw.apply_when_event_risk)
        ...
        : ['CAUTION'],
      min_turbo_score: Math.max(0, this.finiteNumber(raw.min_turbo_score, 0.9)),
      max_tail_risk_score: Math.max(0, this.finiteNumber(raw.max_tail_risk_score, 0.3)),
      require_decision_brain: String(raw.require_decision_brain || 'ENTER_NOW')
        .trim()
        .toUpperCase(),
      ...
      max_probe_entries_per_hour: Math.max(
        1,
        Math.floor(this.finiteNumber(raw.max_probe_entries_per_hour, 1)),
      ),
      min_minutes_between_probe_entries: Math.max(
        0,
        this.finiteNumber(raw.min_minutes_between_probe_entries, 60),
      ),
      max_open_probe_positions: Math.max(
        1,
        Math.floor(this.finiteNumber(raw.max_open_probe_positions, 1)),
      ),
      max_total_open_positions_when_probe: Math.max(
        1,
        Math.floor(this.finiteNumber(raw.max_total_open_positions_when_probe, 2)),
      ),
      block_after_consecutive_losses: Math.max(
        1,
        Math.floor(this.finiteNumber(raw.block_after_consecutive_losses, 2)),
      ),
      block_after_recent_stop_loss_minutes: Math.max(
        0,
        this.finiteNumber(raw.block_after_recent_stop_loss_minutes, 60),
      ),
    };
  }
  ...
  getAegisDecisionEnforcementConfig(): AegisDecisionEnforcementRuntimeConfig {
    ...
    return {
      ...
      block_entry_quality_shadow_block_when_event_risk: {
        ...
        event_modes: Array.isArray(
          raw.block_entry_quality_shadow_block_when_event_risk?.event_modes,
        )
          ? raw.block_entry_quality_shadow_block_when_event_risk.event_modes.map((mode) =>
              this.normalizeEventRiskMode(mode),
            )
          : [],
      },
      event_risk_enforcement: {
        caution_blocks_weak_entries:
          raw.event_risk_enforcement?.caution_blocks_weak_entries === true,
        risk_off_blocks_non_a_plus: raw.event_risk_enforcement?.risk_off_blocks_non_a_plus === true,
        manual_only_blocks_all_new_entries:
          raw.event_risk_enforcement?.manual_only_blocks_all_new_entries === true,
      },
  ...
  getAegisCleanEntryGuardConfig(): AegisCleanEntryGuardRuntimeConfig {
    ...
    return {
      ...
      ignoreRuleGateInsufficientDataWhenModelOk:
        raw.ignore_rule_gate_insufficient_data_when_model_ok !== false,
      ...
      dirtyConditions: {
        blockWhenEntryQualityInsufficient:
          raw.dirty_conditions?.block_when_entry_quality_insufficient !== false,
        blockWhenEventRiskWouldBlock:
          raw.dirty_conditions?.block_when_event_risk_would_block !== false,
        blockWhenTailRiskGte: Math.max(
          0,
          this.finiteNumber(raw.dirty_conditions?.block_when_tail_risk_gte, 0.45),
        ),
        blockWhenEntryQualityNotAllow:
          raw.dirty_conditions?.block_when_entry_quality_not_allow !== false,
      },
      requireCleanForPremiumSymbols: raw.require_clean_for_premium_symbols !== false,
      cleanConditions: {
        requireDecisionBrainEnterNow:
          raw.clean_conditions?.require_decision_brain_enter_now !== false,
        ...
        requireEventRiskWouldBlockFalse:
          raw.clean_conditions?.require_event_risk_would_block_false !== false,
        maxTailRiskScore: Math.max(
          0,
          this.finiteNumber(raw.clean_conditions?.max_tail_risk_score, 0.4),
        ),
      },
      exception: {
        allowExtremeMomentumInShadowOnly:
          raw.exception?.allow_extreme_momentum_in_shadow_only !== false,
  ...
  getEntryQualityGateConfig(_symbol?: string): AegisEntryQualityGateRuntimeConfig {
    ...
    return {
      ...
      config: {
        ...
        minScoreShort: Math.max(0, this.finiteNumber(raw.min_score_short, 0.7)),
        ...
        emaDistanceLimit: Math.max(
          0,
          this.finiteNumber(raw.overextension?.ema_distance_limit, 0.006),
        ),
  ...
  getAegisExitEyeConfig(): AegisExitEyeYamlConfig {
    ...
    return {
      ...
      neutral_votes_to_protect: Math.max(
        0,
        Math.floor(this.finiteNumber(raw.neutral_votes_to_protect, 2)),
      ),
      opposite_votes_to_close: Math.max(
        0,
        Math.floor(this.finiteNumber(raw.opposite_votes_to_close, 2)),
      ),
      min_roe_to_close_on_opposite: this.finiteNumber(raw.min_roe_to_close_on_opposite, 0.06),
      min_peak_roe_to_close_on_opposite: this.finiteNumber(
      ...
      min_peak_roe_to_close_on_neutral: this.finiteNumber(
        raw.min_peak_roe_to_close_on_neutral,
        0.12,
      ),
      min_giveback_to_close_on_neutral: this.finiteNumber(
        raw.min_giveback_to_close_on_neutral,
        0.04,
      ),
      require_consecutive_neutral_close: Math.max(
        1,
        Math.floor(this.finiteNumber(raw.require_consecutive_neutral_close, 2)),
      ),
      require_consecutive_neutral: Math.max(
        1,
        Math.floor(this.finiteNumber(raw.require_consecutive_neutral, 2)),
      ),
      require_consecutive_opposite: Math.max(
        1,
        Math.floor(this.finiteNumber(raw.require_consecutive_opposite, 1)),
      ),
  ...
  getAegisProfitProtectionConfig(): AegisProfitProtectionRuntimeConfig {
    ...
    return {
      ...
      immediate_trigger_buffer_pct: Math.max(
        0,
        this.finiteNumber(raw.immediate_trigger_buffer_pct, 0.001),
      ),
    };
  }
  ...
  private getDefaultConfig(): NinjaYamlConfig {
    return {
      ...
      REGIME_DETECTOR: {
        ...
        bias_strength_threshold: 0.2,
        obi_confirmation_threshold: 0.1,
      },
      IMMUNE_SYSTEM: {
        alpha_slow_rise: 0.15,
        alpha_fast_fall: 0.7,
        panic_exit_threshold: 0.7,
      },
      REGIMES: {
        AEGIS_TURBO: {
          leverage: 15,
      ...
      aegis: {
        ...
        regime_guard: {
          ...
          require_btc_eth_alignment_for_alts:
            DEFAULT_AEGIS_REGIME_GUARD_CONFIG.requireBtcEthAlignmentForAlts,
  ...
  private getAegisSymbolYamlEntries(): Record<string, AegisSymbolEntryYamlConfig> {
    ...
    const profiles = this.asPlainObject(rawSymbols.profiles) as
      | Record<string, AegisSymbolYamlConfig>
      | undefined;
    const explicitEntries = this.asPlainObject(rawSymbols.entries);
    const sourceEntries =
      explicitEntries ||
      Object.fromEntries(
        Object.entries(rawSymbols).filter(
          ([key]) => key !== 'defaults' && key !== 'profiles' && key !== 'entries',
        ),
      );
  ...
  private resolveRegimeSymbolOverride(
    symbol: string,
    regimeKey: string,
  ): RegimeSymbolOverrideYamlConfig | undefined {
    const raw = this.config.SYMBOL_OVERRIDES?.[symbol]?.[regimeKey];
    if (!raw) return undefined;

    const profileName = typeof raw.profile === 'string' ? raw.profile.trim() : '';
    const profile = profileName
      ? this.config.REGIME_PROFILES?.[regimeKey]?.[profileName]
      : undefined;
    return {
      ...(profile || {}),
      ...raw,
  ...
  private normalizeEntryQualityGateMode(
    mode?: AegisEntryQualityGateMode | string,
  ...
  private normalizeDecisionEnforcementMode(
    mode?: AegisDecisionEnforcementMode | string,
  ): AegisDecisionEnforcementMode {
    const normalized = (mode || 'OFF').trim().toUpperCase();
  ...
  private normalizeCleanEntryGuardMode(
    mode?: AegisCleanEntryGuardMode | string,
  ): AegisCleanEntryGuardMode {
    const normalized = (mode || 'SHADOW').trim().toUpperCase();
  ...
  private normalizeRegimeGuardMode(mode?: AegisRegimeGuardMode | string): AegisRegimeGuardMode {
    const normalized = String(mode || 'SHADOW')
      .trim()
      .toUpperCase();
    if (normalized === 'OFF' || normalized === 'SHADOW' || normalized === 'ENFORCE') {
      return normalized;
  ...
  private normalizeRegimeSource(source?: AegisRegimeSource | string): AegisRegimeSource {
    const normalized = String(source || 'HYBRID_HEURISTIC')
      .trim()
      .toUpperCase();
  ...
  private normalizeRegimeList(
    values: Array<AegisRegimeLabel | string> | undefined,
    fallback: AegisRegimeLabel[],
  ): AegisRegimeLabel[] {
  ...
  private normalizeMomentumRideSideConfig(
    ...
    const fallbackRegimes: AegisRegimeLabel[] =
      side === 'LONG'
    ...
    return {
      ...
      leverage: Math.min(
        maxLeverage,
        Math.max(1, this.finiteNumber(merged?.leverage, side === 'LONG' ? 20 : 10)),
      ),
      positionFraction: Math.min(
        maxPositionFraction,
        Math.max(0, this.finiteNumber(merged?.position_fraction, 0.01)),
      ),
      minTurboScore: Math.max(
        0,
        this.finiteNumber(merged?.min_turbo_score, side === 'LONG' ? 0.88 : 0.92),
      ),
      minVolumeRatio: Math.max(
        0,
        this.finiteNumber(merged?.min_volume_ratio, side === 'LONG' ? 1.5 : 1.7),
      ),
      momentumCandles: momentumCandles === 2 ? 2 : 3,
      maxTailRiskScore: Math.max(
        0,
        this.finiteNumber(merged?.max_tail_risk_score, side === 'LONG' ? 0.3 : 0.25),
      ),
      ...
      minCloseLocation: Math.max(
        0.5,
        Math.min(1, this.finiteNumber(merged?.min_close_location, 0.7)),
      ),
  ...
  private normalizeEntryPolicyMode(
    mode?: AegisEntryPolicyMode | string,
    fallback: AegisEntryPolicyMode = 'OFF',
  ): AegisEntryPolicyMode {
    const normalized = String(mode || fallback)
      .trim()
      .toUpperCase();
    if (
      normalized === 'ENFORCE' ||
      normalized === 'SHADOW' ||
      normalized === 'OFF' ||
      normalized === 'ENFORCE_PROBE_LONG_CRITICAL'
    ) {
      return normalized;
    }
    return fallback;
  ...
  private normalizeEntryGuardPolicy(
    ...
    const probeLongCriticalAction = this.normalizeLongRiskAction(
      raw?.probe_long_critical_action,
      fallback.probeLongCriticalAction,
    );
    const probeLongHighAction = this.normalizeLongRiskAction(
      raw?.probe_long_high_action,
      fallback.probeLongHighAction,
    );
    const aegisLongCriticalAction = this.normalizeLongRiskAction(
      raw?.aegis_long_critical_action,
      fallback.aegisLongCriticalAction,
    );
    const minRiskLevelToBlockProbe = this.normalizeLongRiskBlockLevel(
      raw?.min_risk_level_to_block_probe,
      fallback.minRiskLevelToBlockProbe,
    );
  ...
  private normalizeLongRiskAction(
    value: unknown,
    fallback?: 'BLOCK' | 'SHADOW',
  ...
  private normalizeLongRiskBlockLevel(
    value: unknown,
    fallback?: 'HIGH' | 'CRITICAL',
  ): 'HIGH' | 'CRITICAL' | undefined {
    const normalized = String(value || fallback || '')
      .trim()
      .toUpperCase();
