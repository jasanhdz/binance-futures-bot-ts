...
describe('NinjaConfigManager Aegis symbol modes', () => {
  ...
  it('keeps analytical overlays SHADOW while E4 remains the explicit final veto', () => {
    ...
    expect(
      Object.fromEntries(Object.entries(guards).map(([name, guard]) => [name, guard.mode])),
    ).toEqual({
      regime: 'SHADOW',
    ...
    expect(
      Object.entries(guards).every(
        ([name, guard]) =>
          guard.enabled &&
          (name === 'e4_tail_risk' ? guard.mode === 'ENFORCE' : guard.mode === 'SHADOW'),
      ),
    ).toBe(true);
  });
  ...
  it('uses 90 percent of available wallet for every live symbol and side', () => {
    ...
    for (const symbol of ['ETHUSDT', ...massShadowSymbols]) {
      expect(config.getAegisPositionFractionOverride(symbol, 'LONG')?.positionFraction).toBe(0.9);
      expect(config.getAegisPositionFractionOverride(symbol, 'SHORT')?.positionFraction).toBe(0.9);
    }
    expect(config.getAegisTurboConfig()?.position_fraction_cap).toBe(0.9);
  });
...
`,
    ...
    expect(config.getRegimeConfig('AEGIS_TURBO', 'SUIUSDT').hardStopRoe).toBe(-0.4);
    expect(config.getRegimeConfig('AEGIS_TURBO', 'SUIUSDT').entryThreshold).toBe(0.6);
  });
...
symbols:
  ...
  entries:
    ...
    SUIUSDT: {}
`;
    const legacyConfig = new NinjaConfigManager(
      writeConfig(
        symbolsYaml,
        `
  ETHUSDT:
...
`,
    ...
    const dryConfig = new NinjaConfigManager(
      writeConfig(
        symbolsYaml,
        `
  ETHUSDT:
...
`),
  ...
  it('parses entry_policy regime guard OFF, SHADOW and ENFORCE', () => {
    const shadowConfig = new NinjaConfigManager(
      writeConfig(`
  entry_policy:
    enabled: true
    guards:
...
`),
    ...
    const enforceConfig = new NinjaConfigManager(
      writeConfig(`
  entry_policy:
    enabled: true
    guards:
...
`),
    ...
    const offConfig = new NinjaConfigManager(
      writeConfig(`
  regime_guard:
    enabled: true
    mode: SHADOW
...
`),
    ...
    expect(shadowConfig.getAegisEntryPolicyConfig().guards.regime).toEqual({
      enabled: true,
      mode: 'SHADOW',
    });
    expect(enforceConfig.getAegisEntryPolicyConfig().guards.regime).toEqual({
      enabled: true,
      mode: 'ENFORCE',
    });
    expect(offConfig.getAegisEntryPolicyConfig().guards.regime).toEqual({
      enabled: false,
      mode: 'OFF',
    });
  });
...
`),
    ...
    expect(config.getAegisEntryPolicyConfig().guards.regime_context).toEqual({
      enabled: true,
      mode: 'SHADOW',
    });
  });
...
`),
    ...
    expect(momentum.symbols.ETHUSDT.long.maxTailRiskScore).toBe(0.3);
