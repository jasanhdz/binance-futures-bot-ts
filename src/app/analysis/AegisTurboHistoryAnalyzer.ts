...
export async function analyzeAegisTurboHistory(options: AnalyzeOptions): Promise<AnalyzerReport> {
  ...
  for (const date of dates) {
    const loadedSignals = await readJsonl<AegisTurboSignalHistoryInput>(
      path.join(baseDir, `turbo_signals_${date}.jsonl`),
    );
    const loadedTrades = await readJsonl<TradeRecord>(
      path.join(baseDir, `turbo_trades_${date}.jsonl`),
    );
    const loadedEvents = await readJsonl<AegisTurboTradeEventInput>(
      path.join(baseDir, `turbo_trade_events_${date}.jsonl`),
    );
    const loadedSnapshots = await readJsonl<AegisAccountSnapshotInput>(
      path.join(baseDir, `account_snapshots_${date}.jsonl`),
    );
    corruptedLines +=
      loadedSignals.corrupted +
      loadedTrades.corrupted +
      loadedEvents.corrupted +
      loadedSnapshots.corrupted;
...
function buildReport(input: {
  ...
  const roeValues = closes
    .map((trade) => finiteNumber(trade.roe))
  ...
  const startingBalance = firstFinite(
    input.snapshots.map((snapshot) => snapshot.wallet_balance ?? snapshot.available_balance),
  );
  const endingBalance = lastFinite(
    input.snapshots.map((snapshot) => snapshot.wallet_balance ?? snapshot.available_balance),
  );
  ...
  const summary = {
    ...
    range:
      input.dates.length > 1
        ? `${input.dates[0]} to ${input.dates[input.dates.length - 1]}`
        : undefined,
    ...
    avg_duration_minutes: round(
      avg(closes.map((trade) => finiteNumber(trade.duration_minutes)).filter(isNumber)),
    ),
  ...
  return {
    ...
    by_side: groupClosedTrades(
      closedWithOpen,
      (trade) => trade.close.side ?? trade.open?.side ?? 'UNKNOWN',
    ),
    by_score_bucket: buildScoreBuckets(input.signals, closedWithOpen),
    by_exit_reason: groupClosedTrades(closedWithOpen, (trade) =>
      classifyExitReason(trade.close.exit_reason),
    ),
...
function groupClosedTrades(
  ...
  const groups = new Map<
    string,
    Array<{ close: AegisTurboTradeCloseInput; open?: AegisTurboTradeOpenInput }>
  >();
  ...
  for (const [key, values] of groups.entries()) {
    const pnlValues = values.map((row) =>
      numberOrZero(row.close.net_pnl_usdt ?? row.close.pnl_usdt),
    );
    ...
    output[key] = stripUndefined({
      ...
      worst_trade_roe: round(min(roeValues)),
      avg_mfe_roe: round(
        avg(values.map((row) => finiteNumber(row.close.mfe_roe)).filter(isNumber)),
      ),
      avg_mae_roe: round(
        avg(values.map((row) => finiteNumber(row.close.mae_roe)).filter(isNumber)),
      ),
      max_consecutive_losses: maxConsecutiveLosses(pnlValues),
...
function buildScoreBuckets(
  ...
  for (const bucket of SCORE_BUCKETS) {
    ...
    const pnlValues = bucketTrades.map((row) =>
      numberOrZero(row.close.net_pnl_usdt ?? row.close.pnl_usdt),
    );
...
function buildSignalStats(
  ...
  return {
    ...
    top_block_reasons: countTop(
      signals
        .map((signal) => signal.gate_reason ?? signal.gated_blocked_by ?? signal.reason)
...
function buildExitEyeStats(events: AegisTurboTradeEventInput[]): Record<string, unknown> {
  const exitEyeEvents = events.filter((event) =>
    String(event.event || '').startsWith('AEGIS_EXIT_EYE_'),
  );
  const roeValues = exitEyeEvents
    .map((event) => finiteNumber(event.roe ?? event.metadata?.currentRoe))
    .filter(isNumber);
  const givebackValues = exitEyeEvents
    .map((event) => finiteNumber(event.metadata?.givebackRoe))
    .filter(isNumber);
  return {
    exit_eye_shadow_protect_count: exitEyeEvents.filter(
      (event) => event.event === 'AEGIS_EXIT_EYE_SHADOW_PROTECT',
    ).length,
    exit_eye_shadow_close_count: exitEyeEvents.filter(
      (event) => event.event === 'AEGIS_EXIT_EYE_SHADOW_CLOSE',
    ).length,
    exit_eye_close_count: exitEyeEvents.filter(
      (event) => event.event === 'AEGIS_EXIT_EYE_CLOSE_POSITION',
    ).length,
...
function buildPortfolioStats(
  snapshots: AegisAccountSnapshotInput[],
  opens: AegisTurboTradeOpenInput[],
): Record<string, unknown> {
  const maxSimultaneousPositions =
    max(
      snapshots.map((snapshot) => finiteNumber(snapshot.open_positions_count)).filter(isNumber),
    ) ?? 0;
  return {
    ...
    long_exposure_count:
      max(
        snapshots.map((snapshot) => snapshot.portfolio_exposure?.long_symbols).filter(isNumber),
      ) ?? 0,
    short_exposure_count:
      max(
        snapshots.map((snapshot) => snapshot.portfolio_exposure?.short_symbols).filter(isNumber),
      ) ?? 0,
    total_margin_peak: round(
      max(
        snapshots
          .map(
            (snapshot) =>
              snapshot.total_margin_used ?? snapshot.portfolio_exposure?.total_margin_used,
          )
          .filter(isNumber),
      ),
    ),
    total_notional_peak: round(
      max(
        snapshots
          .map((snapshot) => snapshot.total_notional ?? snapshot.portfolio_exposure?.total_notional)
          .filter(isNumber),
      ),
    ),
    daily_loss_events: snapshots.filter(
      (snapshot) => typeof snapshot.daily_pnl_pct === 'number' && snapshot.daily_pnl_pct < 0,
    ).length,
  };
}
...
function renderMarkdown(report: AnalyzerReport, dates: string[]): string {
  ...
  const bySymbolRows =
    Object.entries(report.by_symbol)
      .map(
        ([symbol, row]) =>
          `| ${symbol} | ${row.trades ?? 0} | ${formatPct(row.win_rate)} | ${formatMoney(row.net_pnl)} | ${formatPf(row.profit_factor)} | ${formatPct(row.avg_roe)} | ${row.best_score_bucket ?? 'N/A'} |`,
      )
      .join('\n') || '| N/A | 0 | 0.0% | $0.00 | N/A | 0.0% | N/A |';
  const bucketRows = Object.entries(report.by_score_bucket)
    .map(
      ([bucket, row]) =>
        `| ${bucket} | ${row.signals ?? 0} | ${row.trades ?? 0} | ${formatPct(row.win_rate)} | ${formatPct(row.avg_roe)} | ${formatMoney(row.net_pnl)} |`,
    )
    .join('\n');
...
async function readJsonl<T extends Record<string, unknown>>(
  filePath: string,
): Promise<{ rows: T[]; corrupted: number }> {
  try {
...
function filterSnapshots(
  rows: AegisAccountSnapshotInput[],
  symbol?: string,
): AegisAccountSnapshotInput[] {
  if (!symbol) return rows;
  return rows.map((row) => ({
...
function isOpenTrade(
  trade: TradeRecord,
): trade is AegisTurboTradeOpenInput & { timestamp: string } {
  return trade.status === 'OPEN';
}

function isCloseTrade(
  trade: TradeRecord,
): trade is AegisTurboTradeCloseInput & { timestamp: string } {
  return (
    trade.status === 'CLOSED' &&
    isVerifiedAegisMetricRecord(trade as unknown as Record<string, unknown>)
  );
}
...
function classifyExitReason(reason?: string): string {
  ...
  if (value.includes('EMERGENCY') || value.includes('FAILED') || value.includes('BRACKET'))
    return 'EMERGENCY_CLOSE';
  return 'UNKNOWN';
}

function bestScoreBucket(
  rows: Array<{ close: AegisTurboTradeCloseInput; open?: AegisTurboTradeOpenInput }>,
): string | null {
  ...
  for (const row of rows) {
    ...
    buckets.set(
      bucket,
      (buckets.get(bucket) ?? 0) + numberOrZero(row.close.net_pnl_usdt ?? row.close.pnl_usdt),
    );
  }
...
function countReason(
  ...
  const haystack = [
    ...signals.map(
      (signal) =>
        `${signal.reason ?? ''} ${signal.gate_reason ?? ''} ${signal.gated_blocked_by ?? ''}`,
    ),
