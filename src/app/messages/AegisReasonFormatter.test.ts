...
describe('formatAegisReason', () => {
  it('formats insufficient recent model agreement', () => {
    expect(formatAegisReason('insufficient_recent_model_agreement')).toBe(
  ...
  it('formats short disabled reason', () => {
    expect(formatAegisReason('short_disabled_in_turbo_v010')).toBe(
      'SHORT detectado, pero deshabilitado por configuración',
    );
  });

  it('formats operational gate reasons', () => {
    expect(formatAegisReason('aegis_turbo_yaml_disabled')).toBe(
      'Aegis Turbo deshabilitado en YAML',
    );
    expect(formatAegisReason('aegis_live_disabled')).toBe(
      'Live deshabilitado por variable de entorno',
    );
