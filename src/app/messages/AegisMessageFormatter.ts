...
function formatAegisTurboBlock(input: AegisStartupMessageInput): string {
  ...
  const mode =
    turbo?.enabled === false
      ? 'OFF'
      : (turbo?.mode ?? (input.mode.liveEnabled ? 'LIVE' : 'SHADOW'));
  ...
  const trailing = config.trailingEnabled ? formatPct(trailingActivation, true) : 'OFF';
...
function formatMomentumExamples(examples?: MomentumStartupExample[]): string | undefined {
  ...
  const formatted = Array.from(groups.entries()).map(
    ([key, symbols]) => `${symbols.join('/')} ${key}`,
  );
...
export function formatPositionsMessage(input: {
  activePositions: AegisPositionMessageInput[];
}): string {
  if (input.activePositions.length === 0) {
  ...
  const title =
    input.activePositions.length === 1 ? '💼 **Posición activa**' : '💼 **Posiciones activas**';
  const positions = input.activePositions
    .map((position) => {
      const baseAsset = position.baseAsset ?? baseAssetFromSymbol(position.symbol);
...
export function formatAllSignalsMessage(input: {
  signals: AegisSymbolSignalMessageInput[];
}): string {
