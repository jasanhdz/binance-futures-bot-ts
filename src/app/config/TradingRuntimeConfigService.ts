...
import { AegisProbeModeRuntimeConfig } from '../../strategies/aegis/domain/services/AegisProbeMode';
import {
...
export class TradingRuntimeConfigService {
  ...
  getMicroBurstProvenance(config: ReturnType<typeof parseMicroBurstConfig>) {
    ...
    const cohortId =
      requestedCohort ?? `MB-COHORT-${codeCommitSha.slice(0, 12)}-${configHash.slice(0, 12)}`;
    return {
      codeCommitSha,
      configHash,
      cohortId,
      officialCohortReady: codeCommitSha !== 'UNKNOWN' && cohortId.startsWith('MB-COHORT-'),
    };
  }
  ...
  getAegisCleanEntryGuardConfig(): AegisCleanEntryGuardRuntimeConfig {
    const manager = this.manager as any;
    return typeof manager.getAegisCleanEntryGuardConfig === 'function'
      ? manager.getAegisCleanEntryGuardConfig()
  ...
  getAegisRegimeGuardConfig(): AegisRegimeGuardConfig {
    const manager = this.manager as any;
    return typeof manager.getAegisRegimeGuardConfig === 'function'
      ? manager.getAegisRegimeGuardConfig()
      : DEFAULT_AEGIS_REGIME_GUARD_CONFIG;
  }
  ...
  getAegisEntryPolicyConfig(): AegisEntryPolicyRuntimeConfig {
    ...
    return {
      ...
      guards: {
        ...
        decision_brain: {
          enabled: decisionEnforcement.enabled,
          mode: decisionEnforcement.enabled ? 'ENFORCE' : 'OFF',
        },
        entry_quality: {
        ...
        short_gate: {
          enabled: shortGate.enabled === true,
          mode: shortGate.enabled === true ? 'ENFORCE' : 'OFF',
        },
