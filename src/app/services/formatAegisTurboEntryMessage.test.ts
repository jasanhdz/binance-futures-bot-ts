...
describe('formatAegisTurboReason', () => {
    it('formats known and compact raw agreement reasons', () => {
        expect(formatAegisTurboReason('raw_recent_long_agreement_2_of_3')).toBe('Acuerdo LONG reciente 2/3');
        expect(formatAegisTurboReason('rawrecentlongagreement2of3')).toBe('Acuerdo LONG reciente 2/3');
        expect(formatAegisTurboReason('raw_recent_short_agreement_2_of_3')).toBe('Acuerdo SHORT reciente 2/3');
    });

    it('formats known gate reasons', () => {
        expect(formatAegisTurboReason('insufficient_recent_model_agreement')).toBe('Sin acuerdo suficiente entre modelos recientes');
        expect(formatAegisTurboReason('short_disabled_in_turbo_v010')).toBe('SHORT detectado, pero deshabilitado por configuración');
        expect(formatAegisTurboReason('stale_turbo_snapshot')).toBe('Snapshot Turbo desactualizado');
        expect(formatAegisTurboReason('turbo_score_below_threshold')).toBe('Score por debajo del threshold');
        expect(formatAegisTurboReason('allowed_aegis_turbo_micro_live')).toBe('Señal aprobada por gate Aegis Turbo');
    });
});
