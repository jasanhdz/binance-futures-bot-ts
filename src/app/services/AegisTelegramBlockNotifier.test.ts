...
describe('AegisTelegramBlockNotifier', () => {
    ...
    it('accumulates suppressedCount correctly', () => {
        ...
        expect(notifier.decide(input({ timestamp: 2_000 }), { ...config, summary_threshold: 10 }).suppressedCount).toBe(1);
        expect(notifier.decide(input({ timestamp: 3_000 }), { ...config, summary_threshold: 10 }).suppressedCount).toBe(2);
    });
