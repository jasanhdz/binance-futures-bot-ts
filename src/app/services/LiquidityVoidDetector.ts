...
export class LiquidityVoidDetector {
    ...
    public processDepthUpdate(data: WsDepthUpdate): void {
        ...
        const topLevelConcentration = metrics.bidTotalQty > 0 ? metrics.bidTopQty / metrics.bidTotalQty : 0;
        ...
        if (this.previousBidTotal > 0) {
            const liquidityVelocity = (metrics.bidTotalQty - this.previousBidTotal) / this.previousBidTotal;
            if (liquidityVelocity < -0.5) { // 50% de liquidez bid desapareció -> vacío repentino
                stressScore += 0.3;
            }
        }
