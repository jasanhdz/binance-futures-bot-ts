...
function oppositeSignal(action: 'LONG' | 'SHORT'): AegisTradingSignal {
  return {
    ...
    confidence: 0,
    source: 'AEGIS_TURBO',
    longProb: action === 'LONG' ? 0.7 : 0.1,
    shortProb: action === 'SHORT' ? 0.7 : 0.1,
    neutralProb: 0.2,
    metadata: {
      aegis: {
        turbo: {
          ...
          raw: {
            ...
            votes:
              action === 'SHORT'
                ? { long: 0, short: 2, neutral: 1 }
                : { long: 2, short: 0, neutral: 1 },
...
function makeHarness(options: {
  ...
  const mlService = {
    getSignal: vi.fn(
      async () => options.exitSignal ?? oppositeSignal(side === 'LONG' ? 'SHORT' : 'LONG'),
    ),
    getExitSignal: vi.fn(),
    checkHealth: vi.fn(),
  };
...
describe('TradingService Aegis Exit Eye', () => {
  ...
  it('in PROTECT mode moves stop to protect profit without closing or canceling TP', async () => {
    ...
    expect(notifier.sendMessage).toHaveBeenCalledWith(
      expect.stringContaining('Ganancia protegida'),
    );
  ...
  it('does not mark PROTECT_PROFIT executed when exchange stop placement fails', async () => {
    ...
    expect(notifier.sendAlert).toHaveBeenCalledWith(
      ...
      expect.stringContaining('LINKUSDT LONG'),
    );
  ...
  it('in CLOSE mode closes neutral decay when config explicitly allows it', async () => {
    ...
    expect(notifier.sendMessage).toHaveBeenCalledWith(
      expect.stringContaining('Cierre por pérdida de momentum'),
    );
  });
