...
describe('TelegramCommandRouter', () => {
  ...
  it('blocks unauthorized chats', async () => {
    ...
    await expect(router.handleMessage({ chatId: '999', text: '/status' })).resolves.toBe(
      'Unauthorized.',
    );
  });
  ...
  it('rate limits a second command from the same chat', async () => {
    ...
    now = 1500;
    await expect(router.handleMessage({ chatId: '123', text: '/account' })).resolves.toBe(
      'Espera un momento antes de enviar otro comando.',
    );
  });
  ...
  it('keeps unauthorized /blocks blocked by router', async () => {
    ...
    await expect(router.handleMessage({ chatId: '999', text: '/blocks' })).resolves.toBe(
      'Unauthorized.',
    );
  ...
  it('routes /momentum to handler for authorized chat', async () => {
    ...
    await expect(router.handleMessage({ chatId: '123', text: '/momentum' })).resolves.toBe(
      'MOMENTUM',
    );
