...
export class AegisProbeReportService {
  ...
  async buildReport(input: AegisProbeReportInput): Promise<AegisProbeReportOutput> {
    ...
    const eventRows = await this.readRows(
      'turbo_trade_events',
      windowStartDate,
      windowEndDate,
      warnings,
    ...
    const trades = tradeRows
      .filter(
        (row) =>
          stableText(row.status).toUpperCase() !== 'CLOSED' || isVerifiedAegisMetricRecord(row),
      )
    ...
    for (const sample of evaluations) {
      ...
      for (const reason of sample.cleanEntryReasons.length > 0
        ? sample.cleanEntryReasons
        : ['UNKNOWN']) {
    ...
    const mismatches = detectExitReasonLabelMismatches(tradeRows, tradeEventsById).filter(
      (mismatch) => !symbolFilter || mismatch.symbol === symbolFilter,
    );
    ...
    return {
      ...
      avgMaeRoe: avgMae,
      mfeMaeRatio:
        avgMfe !== null && avgMae !== null && avgMae !== 0 ? avgMfe / Math.abs(avgMae) : null,
      ...
      stopLossCount: closedTrades.filter((trade) =>
      ...
      trades: [...trades]
        .sort(
          (a, b) =>
            Date.parse(b.openedAt ?? b.closedAt ?? '') - Date.parse(a.openedAt ?? a.closedAt ?? ''),
        )
        .slice(0, 20),
...
export function detectExitReasonLabelMismatches(
  ...
  const derivedEventsByTradeId =
    eventsByTradeId.size > 0
      ? eventsByTradeId
  ...
  for (const row of tradeRows) {
    ...
    const closeEvents = (derivedEventsByTradeId.get(tradeId) ?? []).filter(
      (event) => stableText(event.event) === 'TRADE_CLOSED',
    );
    ...
    const mismatchReason = classifyExitMismatch(
      reason,
      exitType,
      derivedEventsByTradeId.get(tradeId) ?? [],
    );
    if (mismatchReason) {
      mismatches.push({
        tradeId,
...
function extractProbeEvaluation(row: JsonRecord): AegisProbeEvaluationSample | undefined {
  ...
  return {
    ...
    symbol: stableText(
      row.symbol ?? metadata.symbol ?? trace.symbol ?? probeMode.symbol,
    ).toUpperCase(),
    ...
    eventRiskMode: nullableText(
      probeMode.eventRiskMode ?? cleanEntry.eventRiskMode ?? eventRisk.mode,
    ),
    eventRiskReason: nullableText(
      probeMode.eventRiskReason ?? cleanEntry.eventRiskReason ?? eventRisk.reason,
    ),
    ...
    decisionBrain: nullableText(
      probeMode.decisionBrain ?? cleanEntry.decisionBrain ?? decisionBrain.decisionBrainDecision,
    ),
    entryQualityRecommendation: nullableText(
      probeMode.entryQualityRecommendation ??
        cleanEntry.entryQualityRecommendation ??
        decisionBrain.entryQualityRecommendation ??
        entryQuality.action,
    ),
    entryQualityScore: finiteNumber(
      cleanEntry.entryQualityScore ??
        decisionBrain.entryQualityScore ??
        eventRisk.entryQualityScore,
    ),
    tailRiskScore: finiteNumber(
      probeMode.tailRiskScore ??
        cleanEntry.tailRiskScore ??
        decisionBrain.tailRiskScore ??
        eventRisk.tailRiskScore,
    ),
    setupGrade: nullableText(
      metadata.setupGrade ??
        metadata.setup_grade ??
        cleanEntry.setupGrade ??
        decisionBrain.setupGrade,
    ),
    turboScore: finiteNumber(
      probeMode.turboScore ?? cleanEntry.turboScore ?? turbo.score ?? metadata.turboScore,
    ),
    votes: normalizeVotes(cleanEntry.votes ?? turbo.votes ?? metadata.votes),
  };
}

function extractProbeTrade(
  row: JsonRecord,
  events: JsonRecord[],
): AegisProbeTradeSample | undefined {
  const metadata = row.metadata ?? {};
  ...
  const first10RoeEvent = firstEventAtOrAbove(events, 0.1, openedMs);
  ...
  const exitType = stableText(
    metadata.canonical_exit_type ??
      metadata.exit_type ??
      closeEvent?.metadata?.canonicalExitType ??
      closeEvent?.metadata?.exitType,
  );
  ...
  return {
    ...
    protectProfitActivated: events.some((event) =>
      stableText(event.event).includes('PROTECT_PROFIT'),
    ),
    trailingActivated: events.some(
      (event) =>
        stableText(event.event) === 'TRAILING_ACTIVATED' ||
        stableText(event.reason) === 'MOVE_SL_TRAILING',
    ),
    exitEyeClose:
      events.some((event) => stableText(event.event) === 'AEGIS_EXIT_EYE_CLOSE_POSITION') ||
    ...
    displayExitLabel: nullableText(
      metadata.display_exit_label ?? closeEvent?.metadata?.displayExitLabel,
    ),
    ...
    entryQualityRecommendation: nullableText(
      probeMode.entryQualityRecommendation ?? cleanEntry.entryQualityRecommendation,
    ),
...
function classifyExitMismatch(
  ...
  if (
    (normalizedReason.includes('TAKE_PROFIT') || normalizedReason === 'SL/TP') &&
    normalizedExitType.includes('EXIT_EYE')
  ) {
    return 'take_profit_reason_with_exit_eye_exit_type';
  }
  const trailingEventSeen = events.some(
    (event) =>
      stableText(event.event) === 'TRAILING_ACTIVATED' ||
      stableText(event.reason) === 'MOVE_SL_TRAILING',
  );
...
function formatProbeReportMessages(
  report: AegisProbeReportOutput,
  input: AegisProbeReportInput,
): string[] {
  ...
  if (input.mode === 'near-miss') {
    ...
    appendProbeEvaluations(
      lines,
      report.evaluations.filter((sample) => sample.decision === 'ALLOW'),
    );
    appendWarnings(lines, report.warnings);
    return splitTelegramMessage(lines);
  }
...
function appendProbeTrades(lines: string[], trades: AegisProbeTradeSample[]): void {
  ...
  trades.slice(0, 5).forEach((trade, index) => {
    ...
    lines.push(
      `Score ${formatScore(trade.turboScore)} | TailRisk ${formatScore(trade.tailRiskScore)} | Setup ${trade.setupGrade ?? 'UNKNOWN'} | Result ${formatPct(trade.roe, true)}`,
    );
    lines.push(
      `   MFE ${formatPct(trade.mfeRoe, true)} / MAE ${formatPct(trade.maeRoe, true)} | Exit ${trade.exitReason ?? 'N/D'} ${trade.canonicalExitType ? `(${trade.canonicalExitType})` : ''}`,
    );
...
function appendProbeEvaluations(lines: string[], samples: AegisProbeEvaluationSample[]): void {
  ...
  samples.slice(0, 8).forEach((sample) => {
    lines.push(
      `• ${sample.symbol} ${sample.side} | ${sample.probeReason} | ER ${sample.eventRiskReason ?? 'UNKNOWN'} | Tail ${formatNumber(sample.tailRiskScore, 2)} | Setup ${sample.setupGrade ?? 'UNKNOWN'}`,
    );
  });
}
...
function appendExitMismatchSection(lines: string[], mismatches: ExitReasonLabelMismatch[]): void {
  ...
  for (const mismatch of mismatches.slice(0, 5)) {
    lines.push(
      `• ${mismatch.symbol} ${mismatch.tradeId}: ${mismatch.reason} vs ${mismatch.exitType} (${mismatch.mismatchReason})`,
    );
  }
}
...
function compareProbeTrade(a: AegisProbeTradeSample, b: AegisProbeTradeSample): number {
  return (
    (b.roe ?? Number.NEGATIVE_INFINITY) - (a.roe ?? Number.NEGATIVE_INFINITY) ||
...
function tailRiskBand(value: number | null): string {
  ...
  if (value < 0.2) return '0.00-0.20';
  if (value < 0.25) return '0.20-0.25';
  if (value < 0.3) return '0.25-0.30';
...
function titleFor(input: AegisProbeReportInput): string {
  ...
  return input.symbol
    ? `🧪 Probe Mode ${input.symbol} — última ${window}`
    : `🧪 Probe Mode — última ${window}`;
}
