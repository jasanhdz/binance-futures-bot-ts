...
export class AegisMomentumReportService {
  ...
  async buildReport(input: AegisMomentumReportInput): Promise<AegisMomentumReportOutput> {
    ...
    for (const sample of samples) {
      ...
      const reasons = sample.momentumReasons.filter((reason) =>
        MOMENTUM_DIAGNOSTIC_REASONS.has(reason),
      );
    ...
    const topCandidates = [...samples]
      .filter(
        (sample) =>
          sample.patternDetected ||
          sample.momentumDecision === 'SHADOW_ALLOW' ||
          sample.momentumDecision === 'ALLOW',
      )
    ...
    return {
      ...
      finalStrategyMomentumRide: samples.filter(
        (sample) => sample.finalStrategy === 'momentum_ride',
      ).length,
...
export function parseMomentumCommand(
  args: string[],
): ParsedMomentumCommand | InvalidMomentumCommand {
...
function extractMomentumSample(row: JsonRecord): AegisMomentumSample | undefined {
  ...
  const momentumRide =
    metadata.momentumRide ?? traceMomentumMetadata.momentumRide ?? metadata.momentum_ride ?? {};
  const finalStrategy = nullableText(
    metadata.finalStrategy ?? trace.finalStrategy ?? row.finalStrategy,
  );
  const finalDecision = nullableText(
    metadata.finalDecision ?? trace.finalDecision ?? row.finalDecision,
  );
  ...
  const regimeContext =
    metadata.regimeContext ?? traceMomentumMetadata.regimeContext ?? metadata.regime ?? {};
  const regimeEngineV2 =
    momentumRide.regimeEngineV2 ??
  ...
  const reasons = Array.isArray(momentumRide.reasons)
    ? momentumRide.reasons
        .map((reason: unknown) => stableText(reason))
        .filter((reason: string) => reason !== 'N/D')
    : rawReason !== 'N/D'
      ? [rawReason]
      : [];
  ...
  const sample: AegisMomentumSample = {
    ...
    tailRiskScore: finiteNumber(
      metadata.tailRiskScore ?? metadata.tail_risk_score ?? metadata.entryPolicy?.tailRiskScore,
    ),
    hasRiskProfile: Object.keys(riskProfile).length > 0,
    researchMode: nullableBoolean(momentumRide.researchMode),
    regimeUsedAsGate: nullableBoolean(
      momentumRide.regimeUsedAsGate ?? traceMomentumMetadata.regimeUsedAsGate,
    ),
    regimeIgnoredForEntry: nullableBoolean(
      momentumRide.regimeIgnoredForEntry ?? traceMomentumMetadata.regimeIgnoredForEntry,
    ),
    regimeIgnoreReason: nullableText(
      momentumRide.regimeIgnoreReason ?? traceMomentumMetadata.regimeIgnoreReason,
    ),
    ...
    eventRiskMode: nullableText(
      traceMomentumMetadata.eventRiskMode ??
        metadata.eventRiskMode ??
        metadata.entryPolicy?.eventRiskMode,
    ),
    eventRiskWouldBlock: nullableBoolean(
      traceMomentumMetadata.eventRiskWouldBlock ??
        metadata.eventRiskWouldBlock ??
        metadata.entryPolicy?.eventRiskWouldBlock,
    ),
...
function formatMomentumReportMessages(
  report: AegisMomentumReportOutput,
  input: AegisMomentumReportInput,
): string[] {
...
function appendMomentumSamples(lines: string[], samples: AegisMomentumSample[]): void {
  ...
  samples.slice(0, 5).forEach((sample, index) => {
    ...
    lines.push(
      `Pattern: ${sample.candlesCount ?? 'N/D'} candles | Vol: ${formatRatio(sample.volumeRatio)} | Regime: ${sample.regimeLabel ?? 'UNKNOWN'} | Turbo: ${sample.turboAgreement === true ? 'confirmed' : 'not_confirmed'} | Tail: ${formatNumber(sample.tailRiskScore)} | Decision: ${sample.momentumDecision}`,
    );
    lines.push(
      `   Research: ${sample.researchMode === true ? 'ON' : sample.researchMode === false ? 'OFF' : 'N/D'} | Regime gate: ${sample.regimeIgnoredForEntry === true ? 'ignored' : sample.regimeUsedAsGate === true ? 'active' : 'N/D'} | Regime observed: ${sample.regimeMomentumEnvironment ?? sample.regimeObserved ?? 'UNKNOWN'} | EventRisk: ${sample.eventRiskMode ?? 'N/D'}${sample.eventRiskWouldBlock === true ? ' would_block' : ''}`,
    );
    lines.push(
      `   Risk: position_fraction ${formatPercent(sample.positionFraction)} | leverage ${formatNumber(sample.leverage)} | source ${sample.riskProfileSource ?? 'N/D'}`,
    );
...
function titleFor(input: AegisMomentumReportInput): string {
  ...
  if (input.mode === 'detail')
    return `🎢 Momentum Ride ${input.symbol ?? 'Aegis'} — última ${window}`;
...
function isAegisFallbackAfterMomentumDeny(sample: AegisMomentumSample): boolean {
  return (
    sample.finalStrategy === 'aegis_turbo' &&
...
function interpretationFor(sample: AegisMomentumSample): string {
  ...
  if (sample.momentumDecision === 'ALLOW' && sample.finalDecision !== 'ALLOW')
    return 'Momentum permitió, pero EntryPolicy final no abrió';
  if (sample.momentumDecision === 'DENY' || sample.momentumDecision === 'SHADOW_DENY')
    return 'Momentum no aplicó';
  return 'Diagnóstico Momentum';
}

function compareMomentumCandidate(a: AegisMomentumSample, b: AegisMomentumSample): number {
  const decisionDiff =
    momentumDecisionRank(b.momentumDecision) - momentumDecisionRank(a.momentumDecision);
...
function firstReason(value: unknown): string | undefined {
  return Array.isArray(value)
    ? value.map(stableText).find((reason) => reason !== 'N/D')
    : undefined;
}
