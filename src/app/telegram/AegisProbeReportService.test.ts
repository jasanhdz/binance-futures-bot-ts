...
import {
  AegisProbeReportService,
  detectExitReasonLabelMismatches,
} from './AegisProbeReportService';
...
async function writeEvents(rows: Array<Record<string, unknown> | string>): Promise<void> {
  ...
  await fs.writeFile(
    path.join(tempDir, 'turbo_trade_events_2026-05-22.jsonl'),
    `${lines}\n`,
    'utf8',
  );
}
...
describe('AegisProbeReportService', () => {
  ...
  it('calcula win/loss y métricas de trades cerrados ligados a probe_mode_allowed', async () => {
    await writeEvents([
      probeAllowed(),
      {
        timestamp: '2026-05-22T01:08:00.000Z',
        trade_id: 'probe-1',
        symbol: 'AVAXUSDT',
        event: 'BREAK_EVEN_ARMED',
        roe: 0.08,
      },
      {
        timestamp: '2026-05-22T01:12:00.000Z',
        trade_id: 'probe-1',
        symbol: 'AVAXUSDT',
