...
function normalizeEventRiskMode(
  mode?: string,
): 'NORMAL' | 'CAUTION' | 'RISK_OFF' | 'MANUAL_ONLY' | undefined {
...
function signalInputFromTurbo(symbol: string, turbo: any): AegisSymbolSignalMessageInput {
  ...
  return {
    ...
    freshnessIsFresh:
      typeof freshness?.is_fresh === 'boolean'
        ? freshness.is_fresh
...
export class TelegramCommandHandlers implements TelegramCommandHandlersPort {
  ...
  async handleAccount(): Promise<string> {
    ...
    const dailyPnlPct =
      runtime.dailyPnlPct ??
      (finiteNumber(account.walletBalance) &&
      finiteNumber(runtime.dailyStartBalance) &&
      runtime.dailyStartBalance > 0
        ? (account.walletBalance - runtime.dailyStartBalance) / runtime.dailyStartBalance
        : undefined);
    return (
      `${formatAccountMessage(account)}\n` +
  ...
  async handleSignal(symbol?: string): Promise<string> {
    ...
    try {
      ...
      return (
        `${formatSymbolSignalMessage(signalInputFromTurbo(normalized, turbo))}\n` +
      ...
      this.deps.logger?.warn('telegram_signal_failed', {
        symbol: normalized,
        error: String(error),
      });
    ...
    for (const symbol of this.getActiveAegisSymbols()) {
      try {
        ...
        const brainText = brain
          ...
          : '';
        rows.push(
          `${symbol} | ${gated?.action ?? raw?.action ?? 'HOLD'} | score ${formatPct(raw?.turbo_score ?? turbo?.turbo_score)} | salida única L=${votes.long ?? 0} S=${votes.short ?? 0} N=${votes.neutral ?? 0} (sin consenso) | ${fresh}${eqText}${eventRiskText}${brainText}`,
        );
      } catch {
  ...
  async handleRisk(): Promise<string> {
    ...
    const equityBase =
      finiteNumber(account.equityTotal) && account.equityTotal > 0
        ? account.equityTotal
        : account.walletBalance;
    const marginUsedPct =
      finiteNumber(equityBase) && equityBase > 0 ? marginUsed / equityBase : undefined;
    const notionalToEquity =
      finiteNumber(equityBase) && equityBase > 0 ? notional / equityBase : undefined;
    ...
    const cooldownActive =
      finiteNumber(cooldownMs) && finiteNumber(sinceExitMs) && sinceExitMs < cooldownMs;
  ...
  async handleBrackets(): Promise<string> {
    ...
    const blocks = positions.map(({ position, orders }) => {
      ...
      return (
        `🛡️ **${position.symbol}** | **${position.side}**\n` +
  ...
  getActiveAegisSymbols(): string[] {
    const manager = this.deps.configManager as any;
    const configured =
      typeof manager.getActiveAegisSymbols === 'function'
  ...
  private getConfiguredAegisSymbolModes(): string[] {
    ...
    if (typeof manager.getAegisSymbolConfigs === 'function') {
      const configs = Object.values(manager.getAegisSymbolConfigs() || {}) as Array<{
        symbol: string;
        mode: string;
      }>;
      return configs
    ...
    return this.getActiveAegisSymbols().map((symbol) => {
      const mode =
        typeof manager.getSymbolMode === 'function' ? manager.getSymbolMode(symbol) : 'LIVE';
      return `${symbol} ${mode}`;
    });
  }
  ...
  private async readAccount(): Promise<AegisAccountMessageInput & { unrealizedPnlTotal?: number }> {
    const snapshot =
      typeof this.deps.exchange.getUSDTAccountSnapshot === 'function'
  ...
  private async readPositionsWithOrders(): Promise<
    Array<{
      position: {
        symbol: string;
        side: Side;
    ...
    for (const symbol of this.getActiveAegisSymbols()) {
      ...
      for (const side of ['LONG', 'SHORT'] as Side[]) {
        ...
        const margin =
          ...
          (position.entryPrice > 0 && position.leverage > 0
        ...
        const orders = await this.deps.exchange
          .listCloseOrdersForSide(symbol, side)
          .catch(() => []);
        const sameStatePosition =
          state.lastSide === side && (state.mode === 'LONG_RIDE' || state.mode === 'SHORT_RIDE');
        output.push({
          position: {
            symbol,
            side,
            ...
            durationHours:
              sameStatePosition && state.lastEntryAt
  ...
  private async readTradeStatus(symbol: string): Promise<TradeStatusDetails | null> {
    ...
    const preferredSides: Side[] =
      state.lastSide === 'SHORT' ? ['SHORT', 'LONG'] : ['LONG', 'SHORT'];
    ...
    for (const side of preferredSides) {
      ...
      const margin =
        position.isolatedMargin ??
        (entryPrice > 0 && leverage > 0 && quantity > 0
      ...
      const trailingActivationRoe =
        state.lastTrailingActivationRoe ?? regime?.trailingActivationRoe ?? 0.15;
      const trailingActive =
        finiteNumber(state.lastTrailStop) ||
        (finiteNumber(currentRoe) && currentRoe >= trailingActivationRoe) ||
        (finiteNumber(peakRoe) && peakRoe >= trailingActivationRoe);
      const protectProfitEligible =
        profitProtection.enabled === true &&
  ...
  private formatTradeStatus(status: TradeStatusDetails): string {
    const bracketWarning =
      !status.hasSL || !status.hasTP ? `\n🚨 **WARNING:** bracket faltante` : '';
    const beText = status.breakEvenExecuted ? 'ejecutado' : status.breakEvenArmed ? 'armado' : 'no';
  ...
  private signalFromPrediction(
    symbol: string,
    prediction: AegisPredictionResponse,
  ): AegisTradingSignal {
    return {
      symbol,
      action: 'PASS',
  ...
  private formatEventRiskConfigLine(eventRisk: any): string {
    return (
      `🌐 Event Risk: **${boolText(eventRisk?.enabled)}** | mode **${eventRisk?.mode ?? 'NORMAL'}** | enforce **${boolText(eventRisk?.enforce)}** | manual **${boolText(eventRisk?.manual_override_enabled)}**\n` +
  ...
  private formatEventRiskRiskBlock(eventRisk: any): string {
    return (
      `🌐 Event Risk: **${eventRisk?.mode ?? 'NORMAL'}** | enabled **${boolText(eventRisk?.enabled)}** | enforce **${boolText(eventRisk?.enforce)}** | manual override **${boolText(eventRisk?.manual_override_enabled)}**\n` +
