...
function makeHandlers(overrides: Record<string, any> = {}) {
  const symbolModes =
    overrides.symbolModes ??
    Object.fromEntries(
      (overrides.symbols ?? ['ETHUSDT']).map((symbol: string) => [symbol, 'LIVE']),
    );
  ...
  const mlService = {
    getAegisPrediction: vi.fn(async (symbol: string) =>
      prediction(symbol, symbol === 'BTCUSDT' ? 0.284 : 0.632),
    ),
    getSignal: vi.fn(),
    getExitSignal: vi.fn(),
  ...
  const configManager = {
    ...
    getAegisSymbolConfigs: vi.fn(() =>
      Object.fromEntries(
        Object.entries(symbolModes).map(([symbol, mode]) => [
          symbol,
    ...
    getAegisPortfolioRiskConfig: vi.fn(
      () =>
        overrides.portfolioRisk ?? {
          enabled: false,
        },
    ),
    getAegisShortGateConfig: vi.fn(
      () =>
        overrides.shortGate ?? {
          enabled: true,
          mode: 'PREMIUM_ONLY',
...
describe('TelegramCommandHandlers', () => {
  ...
  it('/trade SYMBOL without active position responds without error', async () => {
    ...
    await expect(handlers.handleTrade('LINKUSDT')).resolves.toBe(
      'No hay posición activa para LINKUSDT',
    );
  });
  ...
  it('/blocks inválido muestra ayuda compacta', async () => {
    const blocksReportService = {
      buildTelegramMessages: vi.fn(async () => ['Uso: /blocks [15m|30m|1h|2h|4h|6h|12h|24h]']),
    };
    const { handlers } = makeHandlers({ blocksReportService });
