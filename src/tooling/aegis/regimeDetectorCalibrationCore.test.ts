...
import { DEFAULT_TECHNICAL_REGIME_THRESHOLDS, DeepRegimeMetricRow } from './regimeDetectorDeepAuditCore';
import {
...
function calibratedBucket(overrides: Partial<RegimeBucketCalibration> = {}): RegimeBucketCalibration {
    return {
        symbol: 'ETHUSDT',
        regime: 'BREAKOUT_UP',
        side: 'LONG',
...
describe('regimeDetectorCalibrationCore', () => {
    it('clasifica buckets strong_allow, allow, avoid e insufficient_data', () => {
        ...
        expect(classifyBucket(metricRow({ count: 45, mfeMaeRatio: 1.12, hit8BeforeMinus5Rate: 0.31, worstMaeP90: 0.2 }))).toBe('allow');
        expect(classifyBucket(metricRow({ mfeMaeRatio: 0.9, avgForwardReturnRoe: 0.01, hit8BeforeMinus5Rate: 0.25 }))).toBe('avoid');
    ...
    it('crea ventanas walk-forward sin leakage entre train y test', () => {
        const windows = buildWalkForwardWindows('2026-05-01T00:00:00.000Z', '2026-05-22T00:00:00.000Z', 10, 5, 5);
