...
export type RegimeCalibrationReport = {
    generatedAt: string;
    options: Required<Pick<RegimeCalibrationOptions, 'from' | 'to' | 'trainDays' | 'testDays' | 'stepDays' | 'sampleEvery' | 'leverage' | 'limitGrid'>> & RegimeCalibrationOptions;
    baseline?: {
...
export async function calibrateRegimeDetector(options: RegimeCalibrationOptions = {}): Promise<RegimeCalibrationReport> {
    const generatedAt = new Date().toISOString();
    const symbols = options.symbols?.length ? options.symbols.map((symbol) => symbol.toUpperCase()) : DEFAULT_DEEP_REGIME_SYMBOLS;
    ...
    const calibrationThresholds = { ...DEFAULT_CALIBRATION_THRESHOLDS, ...(options.thresholds ?? {}) };
    const baseAuditOptions: DeepRegimeAuditOptions = {
        candlesDbPath: options.candlesDbPath ?? '/home/jasan/Develop/trading_system/data/binance_candles.db',
    ...
    const report: RegimeCalibrationReport = {
        ...
        baseline: {
            previousGlobalFindings: await loadPreviousFindings(options.reportsDir ?? '/home/jasan/Develop'),
    ...
    if (options.writeReports !== false) {
        report.outputFiles = await writeCalibrationReports(report, options.reportsDir ?? '/home/jasan/Develop');
    }
    return report;
}

export function classifyBucket(row: DeepRegimeMetricRow, thresholds: CalibrationThresholds = DEFAULT_CALIBRATION_THRESHOLDS): BucketClassification {
    ...
    if (row.count >= thresholds.strongMinCount && ratio >= thresholds.strongMinMfeMae && hit8 >= thresholds.strongMinHit8 && avgReturn > 0 && p90Mae <= thresholds.strongMaxP90Mae) return 'strong_allow';
    if (row.count >= thresholds.allowMinCount && ratio >= thresholds.allowMinMfeMae && hit8 >= thresholds.allowMinHit8 && avgReturn >= 0) return 'allow';
    if (ratio < 0.80 && avgReturn < 0 && hit8 < thresholds.avoidMaxHit8) return 'strong_avoid';
    if (ratio < thresholds.avoidMaxMfeMae || avgReturn < 0 || hit8 < thresholds.avoidMaxHit8) return 'avoid';
    return 'neutral';
}

export function buildWalkForwardWindows(from: string, to: string, trainDays: number, testDays: number, stepDays: number): Array<{ trainFrom: string; trainTo: string; testFrom: string; testTo: string }> {
    const fromMs = Date.parse(from);
    const toMs = Date.parse(to);
    const day = 24 * 60 * 60_000;
    const windows = [];
    for (let trainFromMs = fromMs; trainFromMs + (trainDays + testDays) * day <= toMs; trainFromMs += stepDays * day) {
...
async function runWalkForward(input: {
    ...
    const windows = buildWalkForwardWindows(input.from, input.to, input.trainDays, input.testDays, input.stepDays);
    const folds: WalkForwardFold[] = [];
    for (let index = 0; index < windows.length; index += 1) {
        const window = windows[index];
        const train = await auditRegimeDetectorDeep({ ...input.auditOptions, from: window.trainFrom, to: window.trainTo, thresholds: input.thresholds });
        const test = await auditRegimeDetectorDeep({ ...input.auditOptions, from: window.testFrom, to: window.testTo, thresholds: input.thresholds });
        ...
        const selected = trainBuckets.filter((bucket) => bucket.horizon === '60m' && (bucket.classification === 'strong_allow' || bucket.classification === 'allow'));
        const selectedKeys = new Set(selected.map(bucketKey));
        const testSelected = testBuckets.filter((bucket) => bucket.horizon === '60m' && selectedKeys.has(bucketKey(bucket)));
        const trainEdge = average(selected.map((bucket) => bucket.avgForwardReturnRoe).filter(isNumber));
        const testEdge = average(testSelected.map((bucket) => bucket.avgForwardReturnRoe).filter(isNumber));
        const stableBuckets = testSelected
            .filter((bucket) => bucket.classification === 'strong_allow' || bucket.classification === 'allow')
            .map(bucketKey)
            .slice(0, 20);
        ...
        folds.push({
            ...
            trainEdge,
            testEdge,
            edgeDecay: trainEdge !== undefined && testEdge !== undefined ? testEdge - trainEdge : undefined,
            stableBuckets,
            misleadingBuckets,
...
function classifyBuckets(rows: DeepRegimeMetricRow[], thresholds: CalibrationThresholds): RegimeBucketCalibration[] {
    return rows
...
export function buildMomentumRecommendations(buckets: RegimeBucketCalibration[], folds: WalkForwardFold[]): MomentumRegimeRecommendation[] {
    ...
    for (const symbol of symbols) {
        for (const side of ['LONG', 'SHORT'] as Side[]) {
            const rows = buckets.filter((bucket) => bucket.symbol === symbol && bucket.side === side && bucket.horizon === '60m');
            const allowed = rows.filter((row) => (row.classification === 'strong_allow' || row.classification === 'allow') && stableKeys.has(bucketKey(row)));
            const avoid = rows.filter((row) => row.classification === 'avoid' || row.classification === 'strong_avoid');
            if (allowed.length === 0 && rows.every((row) => row.count < DEFAULT_CALIBRATION_THRESHOLDS.insufficientCount)) continue;
...
function thresholdGrid(): TechnicalRegimeThresholds[] {
    ...
    const atr = [0.70, 0.80, 0.90];
    const exhaustion = [0.50, 0.60, 0.70];
...
function scoreCandidate(thresholds: TechnicalRegimeThresholds, folds: WalkForwardFold[]): RegimeCalibrationReport['thresholdCandidates'][number] {
...
export async function writeCalibrationReports(report: RegimeCalibrationReport, reportsDir: string): Promise<RegimeCalibrationReport['outputFiles']> {
    ...
    const recommendationsMarkdown = path.join(reportsDir, `aegis_regime_momentum_recommendations_${stamp}.md`);
...
export function renderCalibrationMarkdown(report: RegimeCalibrationReport): string {
    const best = report.buckets.filter((bucket) => bucket.horizon === '60m' && (bucket.classification === 'strong_allow' || bucket.classification === 'allow'))
        .sort((a, b) => (b.avgForwardReturnRoe ?? -Infinity) - (a.avgForwardReturnRoe ?? -Infinity))
        .slice(0, 20);
    const worst = report.buckets.filter((bucket) => bucket.horizon === '60m' && (bucket.classification === 'avoid' || bucket.classification === 'strong_avoid'))
    ...
    return [
        ...
        '',
        ...report.recommendations.map((row) => `- ${row.symbol} ${row.side}: allow [${row.allowedRegimes.join(', ') || 'none'}], avoid [${row.avoidRegimes.join(', ') || 'none'}], confidence=${row.confidence}. ${row.reason}`),
        '',
        '## Do Not Change Yet',
        '',
...
function renderBucketsCsv(rows: RegimeBucketCalibration[]): string {
    const header = ['symbol', 'regime', 'side', 'horizon', 'count', 'avgForwardReturnRoe', 'medianForwardReturnRoe', 'avgMfeRoe', 'avgMaeRoe', 'mfeMaeRatio', 'hit5BeforeMinus5Rate', 'hit8BeforeMinus5Rate', 'hit10BeforeMinus8Rate', 'falseBreakoutRate', 'avgTimeTo5Minutes', 'avgTimeTo8Minutes', 'avgTimeTo10Minutes', 'worstMaeP75', 'worstMaeP90', 'worstMaeP95', 'stabilityScore', 'sampleQualityScore', 'classification'];
    return [header.join(','), ...rows.map((row) => header.map((key) => csvValue((row as unknown as Record<string, unknown>)[key])).join(','))].join('\n');
}

function renderWalkForwardCsv(rows: WalkForwardFold[]): string {
    const header = ['fold', 'trainFrom', 'trainTo', 'testFrom', 'testTo', 'selectedBuckets', 'trainEdge', 'testEdge', 'edgeDecay', 'stableBuckets', 'misleadingBuckets'];
    return [header.join(','), ...rows.map((row) => header.map((key) => csvValue(Array.isArray((row as any)[key]) ? (row as any)[key].join(';') : (row as any)[key])).join(','))].join('\n');
}
...
function markdownBucketTable(rows: RegimeBucketCalibration[]): string {
    ...
    return [
        ...
        '| --- | ---: | ---: | ---: | ---: | ---: | --- |',
        ...rows.map((row) => `| ${bucketKey(row)} | ${row.count} | ${formatPct(row.avgForwardReturnRoe)} | ${formatNumber(row.mfeMaeRatio)} | ${formatPct(row.hit8BeforeMinus5Rate)} | ${formatPct(row.worstMaeP90)} | ${row.classification} |`)
    ].join('\n');
}
...
function markdownFoldTable(rows: WalkForwardFold[]): string {
    ...
    return [
        ...
        '| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |',
        ...rows.map((row) => `| ${row.fold} | ${row.trainFrom.slice(0, 10)}..${row.trainTo.slice(0, 10)} | ${row.testFrom.slice(0, 10)}..${row.testTo.slice(0, 10)} | ${row.selectedBuckets} | ${formatPct(row.trainEdge)} | ${formatPct(row.testEdge)} | ${formatPct(row.edgeDecay)} | ${row.stableBuckets.length} | ${row.misleadingBuckets.length} |`)
    ].join('\n');
}
...
async function loadPreviousFindings(reportsDir: string): Promise<string[]> {
    try {
        const files = (await fs.readdir(reportsDir))
            ...
            .sort()
            .slice(-1);
        return files.length > 0 ? [`Compared with previous audit file: ${path.join(reportsDir, files[0])}`] : [];
    } catch {
        return [];
    }
}
...
function stabilityFrom(row: DeepRegimeMetricRow): number {
    ...
    const hitScore = Math.min(1, Math.max(0, ((row.hit8BeforeMinus5Rate ?? 0) - 0.20) / 0.25));
