...
function input(overrides: Partial<TradePathStudyInput> = {}): TradePathStudyInput {
  ...
  const observations: RoeObservation[] = [
    {
      timestamp: '2026-08-23T00:05:00.000Z',
      timestampMs: Date.parse('2026-08-23T00:05:00.000Z'),
      roe: 0.06,
    },
    {
      timestamp: '2026-08-23T00:20:00.000Z',
      timestampMs: Date.parse('2026-08-23T00:20:00.000Z'),
      roe: -0.02,
    },
    {
      timestamp: '2026-08-23T01:00:00.000Z',
      timestampMs: Date.parse('2026-08-23T01:00:00.000Z'),
      roe: -0.12,
    },
    {
      timestamp: '2026-08-23T02:00:00.000Z',
      timestampMs: Date.parse('2026-08-23T02:00:00.000Z'),
      roe: 0.1,
    },
  ];
  return {
...
describe('E4 entry MAE study', () => {
  ...
  it('simulates protection after a profit trigger', () => {
    ...
    expect(result.maeImprovement).toBeCloseTo(0.1);
  });

  it('scales MAE for leverage stress', () => {
    const result = analyzeTradePath(input());
    expect(result.leverageStress.find((row) => row.leverage === 30)?.estimatedMaeRoe).toBeCloseTo(
      -0.24,
    );
    expect(result.leverageStress.find((row) => row.leverage === 30)?.breachesConfiguredStop).toBe(
      false,
    );
  });
});
