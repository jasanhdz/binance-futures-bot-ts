...
export type RecentTradeLossAuditReport = {
  generatedAt: string;
  options: Required<
    Pick<RecentTradeLossAuditOptions, 'repoRoot' | 'logsDir' | 'candlesDbPath' | 'outDir'>
  > &
...
export async function auditRecentLosingTrades(
  options: RecentTradeLossAuditOptions = {},
): Promise<RecentTradeLossAuditReport> {
  ...
  const symbols = (options.symbols?.length ? options.symbols : ['ETHUSDT', 'ADAUSDT']).map(
  ...
  const trades = selected.map((pair) =>
    buildTradeAudit(pair, {
  ...
  const contextCharts =
    options.charts === true ? writeContextChartsSync(candlesBySymbol, trades, outDir) : [];
  ...
  const report: RecentTradeLossAuditReport = {
    ...
    timezoneNote:
      'Logs and SQLite timestamps are treated as UTC. Binance screenshots may be local time; matching was done by trade_id, symbol, side, entry/exit price and nearby timestamp, not by assuming the screenshot timezone.',
    trades,
...
function buildTradeAudit(
  ...
  const entryPolicy =
    open?.metadata?.entryPolicy ??
    entryPolicyEvent?.metadata?.entryPolicy ??
    entryPolicyEvent?.metadata?.trace ??
    entryPolicyEvent?.metadata;
  const side = (open?.side ??
    close?.side ??
    entryPolicy?.side ??
    entryPolicy?.trace?.side ??
    'LONG') as Side;
...
function summarizeTrade(
  pair: TradePair,
  entryPolicy: JsonRecord | undefined,
  side: Side,
): TradeAuditSummary {
  const open = pair.open;
  const close = pair.close;
  ...
  const slippageVsSlPrice =
    slPrice !== undefined && exitPrice !== undefined ? exitPrice - slPrice : undefined;
  const slippageVsSlRoe =
    slippageVsSlPrice !== undefined && entryPrice && leverage
      ? (side === 'LONG' ? slippageVsSlPrice / entryPrice : -slippageVsSlPrice / entryPrice) *
        leverage
      : undefined;
  ...
  return {
    ...
    side,
    finalStrategy:
      entryPolicy?.finalStrategy ??
      entryPolicy?.strategy ??
      open?.metadata?.entryPolicy?.finalStrategy ??
      open?.strategy ??
      close?.strategy ??
      'unknown',
...
function evaluateTradeLongRiskShadow(input: {
  ...
  return evaluateAegisLongRiskShadow({
    ...
    eventRiskMode: stringValue(
      input.filters.cleanEntry.metadata?.eventRiskMode ??
        input.filters.eventRisk.mode ??
        input.filters.eventRisk.metadata?.mode,
    ),
    eventRiskWouldBlock:
      input.filters.eventRisk.wouldBlock ?? input.filters.cleanEntry.metadata?.eventRiskWouldBlock,
    btcAction: actionOf(btc),
    btcScore: num(btc?.turbo_score),
    ethAction: actionOf(eth),
    ethScore: num(eth?.turbo_score),
    regimeLogged: stringValue(
      input.entryPolicy?.regime?.regime ?? input.filters.regime.metadata?.regime,
    ),
    regimeWouldBlock: boolValue(
      input.entryPolicy?.regime?.wouldBlock ?? input.filters.regime.wouldBlock,
    ),
    ...
    marketWeakness: {
      belowEma25:
        num(pre.distanceFromEma25Pct) !== undefined
          ? num(pre.distanceFromEma25Pct)! < 0
          : undefined,
...
function filterFromGuard(guard?: JsonRecord): FilterAudit {
  ...
  return {
    decision:
      guard.decision ??
      metadata.decision ??
      (metadata.allowed === true ? 'ALLOW' : metadata.allowed === false ? 'DENY' : undefined),
...
function diagnoseTrade(
  ...
  for (const [name, filter] of Object.entries(filters)) {
    if (filter.decision?.includes('ALLOW') || filter.reason === 'probe_allowed')
      filtersThatAllowed.push(`${name}:${filter.reason ?? filter.decision}`);
    if (
      filter.wouldBlock ||
      filter.decision?.includes('DENY') ||
      filter.decision?.includes('WAIT') ||
      filter.decision?.includes('SHADOW')
    )
      filtersThatWarned.push(`${name}:${filter.reason ?? filter.decision}`);
  }
  if (entryPolicy?.regime?.wouldBlock)
    filtersThatWarned.push(
      `regimeObserved:${entryPolicy.regime.regime ?? 'unknown'}/${entryPolicy.regime.reason ?? 'would_block'}`,
    );
  if (
    regimeEngineV2 &&
    ['AVOID_MOMENTUM', 'UNKNOWN'].includes(regimeEngineV2.momentumEnvironment)
  ) {
    filtersThatWarned.push(
      `regimeEngineV2:${regimeEngineV2.momentumEnvironment}/${regimeEngineV2.technicalRegime}`,
    );
  }
  ...
  const eventRiskWouldBlock =
    filters.eventRisk.wouldBlock === true ||
    filters.cleanEntry.metadata?.eventRiskWouldBlock === true;
  const probeAllowed = filters.probeMode.reason === 'probe_allowed';
  const regimeWarned =
    entryPolicy?.regime?.wouldBlock === true ||
    ['AVOID_MOMENTUM', 'UNKNOWN'].includes(regimeEngineV2?.momentumEnvironment ?? '');
  const entryWasLate =
    summary.side === 'LONG'
  ...
  if (isLoss && (summary.slippageVsSlRoe ?? 0) < -0.01) {
    addCause('stop_slippage', 'LOW', [`Exit vs SL ROE delta=${pct(summary.slippageVsSlRoe)}`]);
  }
  if (isLoss && (path.mfeRoe ?? 0) < 0.05) {
    addCause(
      'market_reversal_after_valid_signal',
      'MEDIUM',
  ...
  if (causes.length === 0) {
    addCause(
      isLoss ? 'model_wrong_direction' : 'valid_signal_managed_profitably',
      isLoss ? 'LOW' : 'MEDIUM',
      [
  ...
  return {
    ...
    entryWasLate,
    exitActedCorrectly:
      summary.exitType?.includes('STOP LOSS') ||
      summary.exitReason === 'AEGIS_EXIT_EYE_OPPOSITE_SIGNAL',
    notes: [
...
function analyzeMarketPath(
  ...
  const preEntry = {
    ...
    localHigh20: max(
      candles.slice(Math.max(0, entryIndex - 20), entryIndex).map((candle) => candle.high),
    ),
    localLow20: min(
      candles.slice(Math.max(0, entryIndex - 20), entryIndex).map((candle) => candle.low),
    ),
  };
  const tradePath = {
...
function roePath(
  ...
  for (const candle of candles) {
    ...
    for (const threshold of thresholds) {
      ...
      if (threshold >= 0 && highRoe >= threshold)
        timeTo[key] = Math.round((candle.timestamp - entryMs) / 60000);
      if (threshold < 0 && lowRoe <= threshold)
        timeTo[key] = Math.round((candle.timestamp - entryMs) / 60000);
    }
  }
  return {
...
function evaluateRegimeEngineV2(
  symbol: string,
  candles: AuditCandle[],
  entryMs: number,
  contextSignals: { btc?: JsonRecord; eth?: JsonRecord },
...
function signalToMarket(
  signal?: JsonRecord,
): { action?: RegimeEngineV2MarketAction; score?: number } | undefined {
...
function selectTrades(
  ...
  symbols: string[],
  includeControl: string | undefined,
  fromMs: number,
  toMs: number,
  warnings: string[],
): TradePair[] {
  ...
  if (includeControl) {
    const control = Array.from(pairs.values())
      ...
      .sort(
        (a, b) =>
          Math.abs(parseTimestamp(a.close?.closed_at) - midpoint(fromMs, toMs)) -
          Math.abs(parseTimestamp(b.close?.closed_at) - midpoint(fromMs, toMs)),
      )[0];
  ...
  return selected.sort(
    (a, b) =>
      parseTimestamp(a.open?.opened_at ?? a.close?.opened_at) -
      parseTimestamp(b.open?.opened_at ?? b.close?.opened_at),
  );
}

function loadAuditCandles(
  dbPath: string,
  selected: TradePair[],
  fromMs: number,
  toMs: number,
  warnings: string[],
): Map<string, AuditCandle[]> {
  const symbols = unique([...selected.map((pair) => pair.symbol), 'BTCUSDT', 'ETHUSDT']);
  let minMs = fromMs;
  let maxMs = toMs;
...
function findTradeSignal(
  index: ReturnType<typeof indexSignals>,
  tradeId: string,
  symbol: string,
  openedAt?: string,
...
async function writeAuditReports(
  report: RecentTradeLossAuditReport,
  outDir: string,
): Promise<NonNullable<RecentTradeLossAuditReport['outputFiles']>> {
...
export function renderRecentLossAuditCsv(report: RecentTradeLossAuditReport): string {
  const header = [
    ...
    'clean_entry',
    'event_risk',
    'long_risk_shadow_level',
    'long_risk_shadow_score',
    'long_risk_shadow_action',
    'regime_v2',
    'mfe_roe',
    'mae_roe',
    'root_causes',
  ...
  const rows = report.trades.map((trade) => [
    ...
    trade.diagnosis.probableRootCauses
      .map((cause) => `${cause.cause}:${cause.confidence}`)
      .join('|'),
  ]);
...
export function renderRecentLossAuditMarkdown(report: RecentTradeLossAuditReport): string {
  ...
  for (const trade of report.trades) {
    lines.push(
      `- ${trade.summary.symbol} ${trade.summary.side} ${trade.summary.pnlUsdt !== undefined && trade.summary.pnlUsdt >= 0 ? 'control/win' : 'loss'}: ${trade.summary.tradeId}, strategy=${trade.summary.finalStrategy}, pnl=${usd(trade.summary.pnlUsdt)}, roe=${pct(trade.summary.roe)}, root=${trade.diagnosis.probableRootCauses.map((cause) => `${cause.cause}(${cause.confidence})`).join(', ')}`,
    );
    lines.push(
      `  - LongRiskShadow: ${trade.longRiskShadow.decision} ${trade.longRiskShadow.riskLevel} score=${fmt(trade.longRiskShadow.riskScore)} action=${trade.longRiskShadow.suggestedAction}`,
    );
  }
  lines.push('');
  ...
  lines.push('');
  lines.push(
    '| Trade | DecisionBrain | EntryQuality | CleanEntry | EventRisk | RegimeEngineV2 | MomentumRide | BTC/ETH | TailRisk | Exit |',
  );
  lines.push('|---|---|---|---|---|---|---|---|---|---|');
  for (const trade of report.trades) {
    ...
    lines.push(
      [
        ...
        fmt(
          trade.filters.decisionBrain.metadata?.tailRiskScore ??
            trade.filters.cleanEntry.metadata?.tailRiskScore,
        ),
  ...
  for (const trade of report.trades) {
    lines.push(
      `| ${trade.summary.symbol} ${trade.summary.side} | ${trade.longRiskShadow.decision} | ${trade.longRiskShadow.riskLevel} | ${fmt(trade.longRiskShadow.riskScore)} | ${trade.longRiskShadow.suggestedAction} | ${trade.longRiskShadow.reasons.join(', ') || 'n/a'} |`,
    );
  }
  lines.push('');
  ...
  lines.push('');
  lines.push(
    '| Symbol | Result | Strategy | Setup | EventRisk | Regime | First30m MFE | First30m MAE | Full MFE | Full MAE |',
  );
  lines.push('|---|---:|---|---|---|---|---:|---:|---:|---:|');
  for (const row of report.comparison) {
    lines.push(
      `| ${row.symbol} | ${usd(row.pnlUsdt)} / ${pct(row.roe)} | ${row.finalStrategy} | ${row.setupGrade ?? 'n/a'} | ${row.eventRisk ?? 'n/a'} | ${row.regime ?? 'n/a'} | ${pct(row.first30mMfeRoe)} | ${pct(row.first30mMaeRoe)} | ${pct(row.mfeRoe)} | ${pct(row.maeRoe)} |`,
    );
  }
  lines.push('');
...
function appendTradeMarkdown(lines: string[], trade: TradeAudit): void {
  ...
  lines.push(
    `- qty/leverage/positionFraction: ${fmt(trade.summary.quantity)} / ${fmt(trade.summary.leverage)}x / ${pct(trade.summary.positionFraction)}`,
  );
  lines.push(`- pnl/roe: ${usd(trade.summary.pnlUsdt)} / ${pct(trade.summary.roe)}`);
  lines.push(
    `- SL/TP: ${fmt(trade.summary.slPrice)} / ${fmt(trade.summary.tpPrice)}; slippage vs SL=${fmt(trade.summary.slippageVsSlPrice)} (${pct(trade.summary.slippageVsSlRoe)} ROE)`,
  );
  lines.push(
    `- exit: ${trade.summary.exitReason ?? 'n/a'} / ${trade.summary.exitType ?? 'n/a'}; actedCorrectly=${String(trade.diagnosis.exitActedCorrectly)}`,
  );
  lines.push(`- chart: ${trade.chartPath ?? 'n/a'}`);
  lines.push('');
  lines.push(`Filters:`);
  lines.push(
    `- DecisionBrain: ${cell(trade.filters.decisionBrain)}; decision=${trade.filters.decisionBrain.metadata?.decisionBrainDecision ?? 'n/a'} setup=${trade.filters.decisionBrain.metadata?.setupGrade ?? 'n/a'} aPlus=${String(trade.filters.decisionBrain.metadata?.aPlus)}`,
  );
  lines.push(
    `- EntryQuality: ${cell(trade.filters.entryQuality)}; modelScore=${fmt(trade.filters.decisionBrain.metadata?.entryQualityModelScore ?? trade.filters.cleanEntry.metadata?.entryQualityModelScore)} featureStatus=${trade.filters.decisionBrain.metadata?.featureStatus ?? trade.filters.cleanEntry.metadata?.featureStatus ?? 'n/a'}`,
  );
  lines.push(
    `- CleanEntry: ${cell(trade.filters.cleanEntry)}; reasons=${JSON.stringify(trade.filters.cleanEntry.metadata?.reasons ?? [])}`,
  );
  lines.push(
    `- EventRisk: ${cell(trade.filters.eventRisk)}; wouldBlock=${String(trade.filters.eventRisk.wouldBlock ?? trade.filters.cleanEntry.metadata?.eventRiskWouldBlock)} mode=${trade.filters.cleanEntry.metadata?.eventRiskMode ?? trade.filters.eventRisk.metadata?.mode ?? 'n/a'}`,
  );
  lines.push(
    `- Regime logged: ${trade.entryPolicy?.regime?.regime ?? 'n/a'} decision=${trade.entryPolicy?.regime?.decision ?? 'n/a'} wouldBlock=${String(trade.entryPolicy?.regime?.wouldBlock)}`,
  );
  lines.push(
    `- RegimeEngineV2 offline: ${trade.market.regimeEngineV2?.momentumEnvironment ?? 'n/a'} / ${trade.market.regimeEngineV2?.technicalRegime ?? 'n/a'} / transition=${trade.market.regimeEngineV2?.transition.risk ?? 'n/a'} reasons=${JSON.stringify(trade.market.regimeEngineV2?.reasons ?? [])}`,
  );
  lines.push(
    `- MomentumRide: ${trade.entryPolicy?.strategyCandidates?.momentum_ride?.decision ?? trade.filters.momentumRide.decision ?? 'n/a'} / ${trade.entryPolicy?.strategyCandidates?.momentum_ride?.reason ?? trade.filters.momentumRide.reason ?? 'n/a'}`,
  );
  lines.push(
    `- LongRiskShadow: ${trade.longRiskShadow.decision} ${trade.longRiskShadow.riskLevel} score=${fmt(trade.longRiskShadow.riskScore)} action=${trade.longRiskShadow.suggestedAction} reasons=${JSON.stringify(trade.longRiskShadow.reasons)}`,
  );
  lines.push(`- ProbeMode: ${cell(trade.filters.probeMode)}`);
  lines.push('');
  lines.push(`Market path:`);
  lines.push(
    `- pre-entry returns 5/15/30/60m: ${pct(trade.market.preEntry?.return5m)} / ${pct(trade.market.preEntry?.return15m)} / ${pct(trade.market.preEntry?.return30m)} / ${pct(trade.market.preEntry?.return60m)}`,
  );
  lines.push(
    `- entry candle closeLocation=${fmt(trade.market.preEntry?.closeLocation)} wickRatio=${fmt(trade.market.preEntry?.wickRatio)} volumeRatio=${fmt(trade.market.preEntry?.volumeRatio20)} distanceEMA25=${pct(trade.market.preEntry?.distanceFromEma25Pct)}`,
  );
  lines.push(
    `- MFE/MAE: ${pct(trade.market.tradePath?.mfeRoe)} / ${pct(trade.market.tradePath?.maeRoe)}; first30m ${pct(trade.market.tradePath?.first30mMfeRoe)} / ${pct(trade.market.tradePath?.first30mMaeRoe)}`,
  );
  lines.push(
    `- time to -5/-8/-10/-20/-40 ROE: ${trade.market.tradePath?.timeToMinus5RoeMinutes ?? 'n/a'} / ${trade.market.tradePath?.timeToMinus8RoeMinutes ?? 'n/a'} / ${trade.market.tradePath?.timeToMinus10RoeMinutes ?? 'n/a'} / ${trade.market.tradePath?.timeToMinus20RoeMinutes ?? 'n/a'} / ${trade.market.tradePath?.timeToMinus40RoeMinutes ?? 'n/a'} min`,
  );
  lines.push('');
  lines.push(`Root causes:`);
  for (const cause of trade.diagnosis.probableRootCauses) {
    lines.push(
      `- ${cause.cause} (${cause.confidence}) for=[${cause.evidenceFor.join('; ')}] against=[${cause.evidenceAgainst.join('; ')}]`,
    );
  }
}
...
function buildComparisonRows(trades: TradeAudit[]): JsonRecord[] {
  return trades.map((trade) => ({
    ...
    eventRisk:
      trade.filters.cleanEntry.metadata?.eventRiskReason ??
      trade.filters.eventRisk.reason ??
      trade.filters.eventRisk.decision,
    ...
    setupGrade:
      trade.filters.decisionBrain.metadata?.setupGrade ??
      trade.filters.cleanEntry.metadata?.setupGrade,
...
function buildRecommendations(trades: TradeAudit[]): RecentTradeLossAuditReport['recommendations'] {
  ...
  const hasRegimeWarnings = losses.some((trade) =>
    trade.diagnosis.probableRootCauses.some((cause) => cause.cause === 'weak_regime_ignored'),
  );
  const hasEventRisk = losses.some((trade) =>
    trade.diagnosis.probableRootCauses.some((cause) => cause.cause === 'event_risk_underblocking'),
  );
  return {
    now: [
    ...
    shadow: [
      hasRegimeWarnings
        ? 'Shadow rule: no LONG when RegimeEngineV2 is AVOID/UNKNOWN or logged regime shadow wouldBlock, unless later data proves it blocks too many winners.'
        : 'Shadow RegimeEngineV2 avoid/unknown separation by side.',
      hasEventRisk
        ? 'Shadow rule: delay LONG one candle when EventRisk CAUTION wouldBlock is true and entry is allowed only by Probe Mode.'
        : 'Shadow EventRisk CAUTION delay rule.',
...
function writeTradeChartSync(
  symbol: string,
  candles: AuditCandle[],
  summary: TradeAuditSummary,
  regime: RegimeEngineV2Decision | undefined,
  outDir: string,
): string {
  ...
  const window = candles.filter(
    (candle) =>
      candle.timestamp >= entryMs - 6 * 3600000 && candle.timestamp <= closeMs + 3 * 3600000,
  );
...
function writeContextChartsSync(
  candlesBySymbol: Map<string, AuditCandle[]>,
  trades: TradeAudit[],
  outDir: string,
): string[] {
  ...
  const opened = trades
    .map((trade) => parseTimestamp(trade.summary.openedAt))
    .filter(Number.isFinite);
  const closed = trades
    .map((trade) => parseTimestamp(trade.summary.closedAt))
    .filter(Number.isFinite);
  const minMs = Math.min(...opened) - 6 * 3600000;
  const maxMs = Math.max(...closed, ...opened) + 3 * 3600000;
  const safeDate = new Date(Math.max(...closed, ...opened))
    .toISOString()
    .slice(0, 10)
    .replace(/-/g, '');
  const paths: string[] = [];
  for (const symbol of ['BTCUSDT', 'ETHUSDT']) {
    const candles = (candlesBySymbol.get(symbol) ?? []).filter(
      (candle) => candle.timestamp >= minMs && candle.timestamp <= maxMs,
    );
    if (candles.length === 0) continue;
    const file = path.join(
      outDir,
      `aegis_loss_audit_${symbol.slice(0, -4)}_context_${safeDate}_chart.png`,
    );
...
function renderPngChart(
  ...
  const prices = candles.flatMap((candle) =>
    [candle.high, candle.low, candle.ema25, candle.ema99].filter(
  ...
  const volH = 62;
  const x = (ts: number) =>
    pad.l + (maxTs === minTs ? 0 : ((ts - minTs) / (maxTs - minTs)) * plotW);
  const y = (price: number) =>
    pad.t + (maxPrice === minPrice ? 0.5 : (maxPrice - price) / (maxPrice - minPrice)) * plotH;
  ...
  drawSeries(
    img,
    candles.map((c) => [x(c.timestamp), y(c.close)]),
    [225, 225, 225, 255],
  );
  drawSeries(
    img,
    candles.filter((c) => c.ema25 !== undefined).map((c) => [x(c.timestamp), y(c.ema25 as number)]),
    [247, 196, 78, 255],
  );
  drawSeries(
    img,
    candles.filter((c) => c.ema99 !== undefined).map((c) => [x(c.timestamp), y(c.ema99 as number)]),
    [83, 166, 255, 255],
  );
  ...
  if (Number.isFinite(marker.closeMs))
    drawTimeLine(img, x(marker.closeMs), pad.t, plotH, [239, 68, 68, 255]);
  img.text(20, 16, marker.title, [230, 236, 243, 255]);
  img.text(
    20,
    height - 42,
    `white=close yellow=EMA25 blue=EMA99 green=entry red=exit/sl ${marker.regimeLabel ?? ''}`,
    [185, 195, 208, 255],
  );
...
function drawSeries(img: RgbaImage, points: number[][], color: number[]): void {
  for (let i = 1; i < points.length; i++)
    img.line(points[i - 1][0], points[i - 1][1], points[i][0], points[i][1], color);
}
  ...
  text(x: number, y: number, text: string, color: number[]): void {
    ...
    for (const ch of text.slice(0, 150)) {
      ...
      for (let bit = 0; bit < 7; bit++) {
        if ((code >> bit) & 1)
          this.rect(cursor + (bit % 3) * 2, y + Math.floor(bit / 3) * 3, 2, 2, color);
      }
      cursor += 8;
    }
  }
...
function pngChunk(type: string, data: Buffer): Buffer {
  ...
  return Buffer.concat([
    ...
    typeBuf,
    data,
    u32(crc32(Buffer.concat([typeBuf, data]))),
  ]);
}
...
function atr(candles: AuditCandle[], window: number): Array<number | undefined> {
  ...
  return trs.map((_, index) =>
    index + 1 >= window ? avg(trs.slice(index + 1 - window, index + 1)) : undefined,
  );
}
