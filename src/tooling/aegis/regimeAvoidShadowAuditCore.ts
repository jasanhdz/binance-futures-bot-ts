...
export async function auditRegimeAvoidShadow(
  options: RegimeAvoidShadowAuditOptions = {},
): Promise<RegimeAvoidShadowAuditReport> {
...
function buildReport(input: {
  ...
  const evaluations = [...closeById.values()]
    .map((close) =>
      evaluateTrade(close, openById.get(close.trade_id), entryPolicyByTradeId.get(close.trade_id)),
    )
  ...
  const report: RegimeAvoidShadowAuditReport = {
    ...
    byRegime: groupMetrics(
      evaluations,
      (row) => row.regime ?? row.shadow.matchedRegime ?? 'MISSING_REGIME',
    ),
    evaluations,
...
function extractRegime(entryPolicy?: Record<string, unknown>): string | undefined {
  ...
  return (
    stringValue(regimeContext?.label) ??
...
function groupMetrics(
  ...
  return Object.fromEntries(
    [...groups.entries()].map(([key, values]) => [key, buildMetrics(values)]),
  );
}
...
function metricLines(
  metrics: RegimeAvoidShadowMetrics & { corruptedLines?: number; duplicateTradesSkipped?: number },
): string {
  return [
...
function markdownMetricsTable(groups: Record<string, RegimeAvoidShadowMetrics>): string {
  const rows = Object.entries(groups).sort(
    (a, b) => b[1].wouldAvoid - a[1].wouldAvoid || a[0].localeCompare(b[0]),
  );
  ...
  return [
    ...
    ...rows.map(
      ([group, metrics]) =>
        `| ${group} | ${metrics.trades} | ${metrics.wouldAvoid} | ${metrics.avoidedWinners} | ${metrics.avoidedLosers} | ${metrics.wouldAvoidPnLImpact} | ${metrics.netSavedPnlEstimated} | ${metrics.netAvoidedRoe} |`,
    ),
  ].join('\n');
}
...
function buildConclusions(report: RegimeAvoidShadowAuditReport): string[] {
  ...
  if (summary.wouldAvoid === 0)
    return ['No closed trades matched calibrated avoid regimes in this window.'];
  const lines = [
  ...
  const topRegime = Object.entries(report.byRegime).sort(
    (a, b) => b[1].netSavedPnlEstimated - a[1].netSavedPnlEstimated,
  )[0];
  if (topRegime)
    lines.push(
      `Best current avoided-regime contribution: ${topRegime[0]} netSavedPnlEstimated=${topRegime[1].netSavedPnlEstimated}.`,
    );
  return lines;
}
