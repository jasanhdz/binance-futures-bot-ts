...
export type DeepRegimeAuditReport = {
  generatedAt: string;
  options: Required<Pick<DeepRegimeAuditOptions, 'timeframe' | 'leverage' | 'sampleEvery'>> &
...
export async function auditRegimeDetectorDeep(
  options: DeepRegimeAuditOptions = {},
): Promise<DeepRegimeAuditReport> {
  ...
  const report: DeepRegimeAuditReport = {
    ...
    unknownBreakdown: breakdownReasons(
      samples.filter((sample) => sample.technicalRegime === 'UNKNOWN'),
    ),
...
function buildSample(input: {
  ...
  const marketConfirmation = classifyMarketConfirmation(
    input.side,
    input.symbol,
    btcSnapshot,
    ethSnapshot,
...
export function classifyTechnicalRegime(
  ...
  const emaSlope =
    ema7 !== undefined && ema7Series.length > 8
  ...
  const wickAgainstPct = wickAgainst(
    current,
    upStack || breakoutUp ? 'LONG' : downStack || breakoutDown ? 'SHORT' : 'LONG',
  );
  ...
  if (exhaustionRisk >= thresholds.maxExhaustionScore) {
    const direction: Side | 'NONE' =
      upStack || breakoutUp ? 'LONG' : downStack || breakoutDown ? 'SHORT' : 'NONE';
    return baseSnapshot(
      'EXHAUSTION',
      scoreFrom(exhaustionRisk),
      direction,
      'overextension_or_reversal_wick',
      indicators,
    );
  }
  if (
    breakoutUp &&
    (volumeRatio ?? 0) >= thresholds.minVolumeRatioForMomentum &&
    (adx ?? 0) >= Math.max(16, thresholds.minAdxForMomentum - 2) &&
    (choppiness ?? 100) <= thresholds.maxChoppinessForMomentum + 3
  ) {
    return baseSnapshot(
      'BREAKOUT_UP',
      confidence([
        volumeRatioScore(volumeRatio),
        adxScore(adx),
        chopScore(choppiness),
        closeLocation,
      ]),
      'LONG',
      'range_breakout_up_volume_confirmed',
      indicators,
    );
  }
  if (
    ...
    return baseSnapshot(
      ...
      'SHORT',
      'range_breakout_down_volume_confirmed',
      indicators,
    );
  }
  if (
    upStack &&
    (emaSlope ?? 0) > thresholds.emaSlopeThreshold &&
    (adx ?? 0) >= thresholds.minAdxForMomentum &&
    (choppiness ?? 100) <= thresholds.maxChoppinessForMomentum &&
    (volumeRatio ?? 0) >= thresholds.minVolumeRatioForMomentum
  ) {
    return baseSnapshot(
      'MOMENTUM_UP',
  ...
  if (
    ...
    return baseSnapshot(
      ...
      confidence([
        ...
        chopScore(choppiness),
        volumeRatioScore(volumeRatio),
        slopeScore(Math.abs(emaSlope ?? 0)),
      ]),
      'SHORT',
      'ema_stack_down_adx_volume',
      indicators,
    );
  }
  if ((choppiness ?? 0) >= 61.8 || (adx ?? 100) < 18 || (bollingerWidth ?? 1) < 0.01) {
    return baseSnapshot(
      'CHOP',
      confidence([
        chopRiskScore(choppiness),
        1 - (adxScore(adx) ?? 0),
        bollingerWidth !== undefined ? Math.max(0, 1 - bollingerWidth * 50) : 0.4,
      ]),
      'NONE',
      'low_adx_or_high_choppiness',
      indicators,
    );
  }
  if (
    ...
    return baseSnapshot(
      'TREND_UP',
      confidence([adxScore(adx), slopeScore(emaSlope), chopScore(choppiness)]),
      'LONG',
      'ema_stack_up',
      indicators,
    );
  ...
  if (
    ...
    return baseSnapshot(
      'TREND_DOWN',
      confidence([adxScore(adx), slopeScore(Math.abs(emaSlope ?? 0)), chopScore(choppiness)]),
      'SHORT',
      'ema_stack_down',
      indicators,
    );
...
function buildSnapshotsBySymbol(
  ...
  for (const [symbol, candles] of candlesBySymbol.entries()) {
    output.set(
      symbol,
      candles.map((_, index) => classifyTechnicalRegime(candles, index, thresholds)),
    );
  }
  return output;
}
...
function calculateOutcome(
  ...
  return {
    ...
    hit10BeforeMinus8: hitThreshold(future, entry.close, side, leverage, 0.1, 0.08),
    ...
    timeTo10Minutes: timeToTarget(future, entry.close, side, leverage, 0.1),
...
function classifyMarketConfirmation(
  side: Side,
  symbol: string,
  btc?: TechnicalRegimeSnapshot,
  eth?: TechnicalRegimeSnapshot,
): MarketConfirmation {
  const checks = [btc, symbol === 'ETHUSDT' ? undefined : eth].filter(
    (item): item is TechnicalRegimeSnapshot => Boolean(item),
  );
...
function finalRegimeForMomentum(
  side: Side,
  snapshot: TechnicalRegimeSnapshot,
  confirmation: MarketConfirmation,
): 'ALLOW' | 'BLOCK' | 'NEUTRAL' {
  if (
    snapshot.technicalRegime === 'CHOP' ||
    snapshot.technicalRegime === 'UNKNOWN' ||
    snapshot.technicalRegime === 'EXHAUSTION' ||
    snapshot.technicalRegime === 'HIGH_VOL_RISK'
  )
    return 'BLOCK';
  if (!regimeConfirmsSide(snapshot.technicalRegime, side)) return 'BLOCK';
  if (confirmation === 'CONTRADICT') return 'BLOCK';
  return confirmation === 'CONFIRM' || confirmation === 'MIXED' || confirmation === 'NEUTRAL'
    ? 'ALLOW'
    : 'NEUTRAL';
}

function buildMetricRows(
  samples: DeepRegimeSample[],
  mode:
    | 'regime'
    | 'regimeSide'
    | 'symbol'
...
function metricKey(sample: DeepRegimeSample, mode: string): string {
  ...
  if (mode === 'symbolRegimeSide')
    return `${sample.symbol}|${sample.technicalRegime}|${sample.side}`;
...
function metricRow(
  ...
  return {
    ...
    side:
      mode === 'regimeSide'
        ? (parts[1] as Side)
        : mode === 'symbolRegimeSide'
          ? (parts[2] as Side)
          : 'ALL',
    horizon,
    count: samples.length,
    avgForwardReturnRoe: round(avgReturn),
    medianForwardReturnRoe: round(
      median(outcomes.map((outcome) => outcome.forwardReturnRoe).filter(isNumber)),
    ),
    ...
    avgTimeTo5Minutes: round(
      avg(outcomes.map((outcome) => outcome.timeTo5Minutes).filter(isNumber)),
    ),
    avgTimeTo8Minutes: round(
      avg(outcomes.map((outcome) => outcome.timeTo8Minutes).filter(isNumber)),
    ),
    avgTimeTo10Minutes: round(
      avg(outcomes.map((outcome) => outcome.timeTo10Minutes).filter(isNumber)),
    ),
    falseBreakoutRate: booleanRate(outcomes.map((outcome) => outcome.falseBreakout)),
    worstMaeP75: round(percentile(adverseMae, 0.75)),
    worstMaeP90: round(percentile(adverseMae, 0.9)),
...
function buildRecommendations(samples: DeepRegimeSample[]): string[] {
  const rows = buildMetricRows(samples, 'symbolRegimeSide').filter(
    (row) => row.horizon === '60m' && row.count >= 30,
  );
  const good = rows
    .filter((row) => row.conclusion === 'good')
    .sort((a, b) => (b.avgForwardReturnRoe ?? -Infinity) - (a.avgForwardReturnRoe ?? -Infinity))
    .slice(0, 12);
  const bad = rows
    .filter((row) => row.conclusion === 'bad')
    .sort((a, b) => (a.avgForwardReturnRoe ?? Infinity) - (b.avgForwardReturnRoe ?? Infinity))
    .slice(0, 12);
  const unknown =
    samples.filter((sample) => sample.technicalRegime === 'UNKNOWN').length /
...
async function writeReports(
  report: DeepRegimeAuditReport,
  samples: DeepRegimeSample[],
  reportsDir: string,
): Promise<DeepRegimeAuditReport['outputFiles']> {
  ...
  const recommendations = path.join(
    reportsDir,
    `aegis_regime_detector_recommendations_${stamp}.md`,
  );
  await fs.writeFile(markdown, renderMarkdown(report), 'utf8');
  await fs.writeFile(
    json,
    JSON.stringify({ ...report, samplePreview: samples.slice(0, 250) }, null, 2),
    'utf8',
  );
...
function nearestSnapshot(
  rows: TechnicalRegimeSnapshot[] | undefined,
  index: number,
): TechnicalRegimeSnapshot | undefined {
  return rows?.[index];
}
...
function conclusion(
  ...
  avgMfe?: number,
  avgMae?: number,
): DeepRegimeMetricRow['conclusion'] {
  if (count < 30 || avgReturn === undefined || avgMfe === undefined || avgMae === undefined)
    return 'insufficient';
...
function rate(values: Array<HitOutcome | undefined>): number | undefined {
  ...
  return finite.length > 0
    ? round(finite.filter((value) => value === 'TARGET_FIRST').length / finite.length)
    : undefined;
}
...
function calculateChoppiness(candles: CandleRow[]): number | undefined {
  ...
  return range > 0 && trSum > 0
    ? (100 * Math.log10(trSum / range)) / Math.log10(candles.length)
    : undefined;
}
...
function normalizeThresholds(
  input?: Partial<TechnicalRegimeThresholds>,
): TechnicalRegimeThresholds {
  return {
    minAdxForMomentum: finiteOr(
      input?.minAdxForMomentum,
      DEFAULT_TECHNICAL_REGIME_THRESHOLDS.minAdxForMomentum,
    ),
    maxChoppinessForMomentum: finiteOr(
      input?.maxChoppinessForMomentum,
      DEFAULT_TECHNICAL_REGIME_THRESHOLDS.maxChoppinessForMomentum,
    ),
    minVolumeRatioForMomentum: finiteOr(
      input?.minVolumeRatioForMomentum,
      DEFAULT_TECHNICAL_REGIME_THRESHOLDS.minVolumeRatioForMomentum,
    ),
    maxAtrPercentileForAggressive: finiteOr(
      input?.maxAtrPercentileForAggressive,
      DEFAULT_TECHNICAL_REGIME_THRESHOLDS.maxAtrPercentileForAggressive,
    ),
    maxExhaustionScore: finiteOr(
      input?.maxExhaustionScore,
      DEFAULT_TECHNICAL_REGIME_THRESHOLDS.maxExhaustionScore,
    ),
    breakoutLookback: Math.max(
      2,
      Math.floor(
        finiteOr(input?.breakoutLookback, DEFAULT_TECHNICAL_REGIME_THRESHOLDS.breakoutLookback),
      ),
    ),
    emaSlopeThreshold: finiteOr(
      input?.emaSlopeThreshold,
      DEFAULT_TECHNICAL_REGIME_THRESHOLDS.emaSlopeThreshold,
    ),
  };
}
