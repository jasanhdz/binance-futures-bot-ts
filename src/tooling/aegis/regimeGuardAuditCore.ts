...
export async function auditRegimeGuard(options: RegimeAuditOptions = {}): Promise<RegimeAuditReport> {
    ...
    const symbols = (options.symbols?.length ? options.symbols : DEFAULT_REGIME_AUDIT_SYMBOLS).map(symbol => symbol.toUpperCase());
    ...
    for (const date of dates) {
        const loadedSignals = await readJsonl<AegisTurboSignalHistoryInput>(path.join(logsDir, `turbo_signals_${date}.jsonl`));
        const loadedEvents = await readJsonl<AegisTurboTradeEventInput>(path.join(logsDir, `turbo_trade_events_${date}.jsonl`));
        const loadedTrades = await readJsonl<TradeRecord>(path.join(logsDir, `turbo_trades_${date}.jsonl`));
    ...
    if (options.momentumOnly) {
        ...
        const momentumTo = options.to ?? (dates[dates.length - 1] ? `${dates[dates.length - 1]}T23:59:59.999Z` : undefined);
        ...
        for (const pattern of patterns) {
            const signal = nearestTimed(signalIndex.get(pattern.symbol) ?? [], pattern.timestampMs, 10 * 60_000);
    ...
    const report: RegimeAuditReport = {
        ...
        counts: {
            ...
            skippedSignals,
            corruptedLines,
            candlesBySymbol: Object.fromEntries([...candleStore.entries()].map(([symbol, candles]) => [symbol, candles.length]))
        },
        evaluations,
        ...
        inconsistencies: evaluations.filter(evaluation => Boolean(evaluation.indicatorWarning)).slice(0, 50),
...
function evaluateCandidate(input: {
    ...
    const btcSignal = nearestTimed(input.signalIndex.get('BTCUSDT') ?? [], input.timestampMs, 10 * 60_000);
    const ethSignal = nearestTimed(input.signalIndex.get('ETHUSDT') ?? [], input.timestampMs, 10 * 60_000);
...
function buildMomentumCandidates(
    ...
    const candidates: Array<{ symbol: string; timestampMs: number; side: Side; snapshot: MomentumPatternSnapshot }> = [];
    for (const symbol of options.symbols) {
        ...
        for (let index = Math.max(20, options.candles - 1); index < candles.length; index += 1) {
            ...
            const pattern = detectMomentumPattern(candles, index, options.candles, options.minVolumeRatio);
            if (!pattern) continue;
            candidates.push({ symbol, timestampMs: current.timestampMs, side: pattern.side, snapshot: pattern.snapshot });
        }
    }
    return candidates
        .sort((a, b) => a.timestampMs - b.timestampMs || a.symbol.localeCompare(b.symbol) || a.side.localeCompare(b.side))
...
export function detectMomentumPattern(
    ...
    const ascendingCloses = window.every((candle, itemIndex) => itemIndex === 0 || candle.close > window[itemIndex - 1].close);
    const descendingCloses = window.every((candle, itemIndex) => itemIndex === 0 || candle.close < window[itemIndex - 1].close);
    const side: Side | undefined = allGreen && ascendingCloses ? 'LONG' : allRed && descendingCloses ? 'SHORT' : undefined;
    if (!side) return undefined;

    const previousVolumes = candles.slice(Math.max(0, index - 20), index).map(candle => candle.volume);
...
export function calculateOutcome(
    ...
    return {
        ...
        hit15BeforeMinus10: hitThresholdBeforeAdverse(future, entry.close, side, leverage, 0.15, 0.10)
    };
}
...
export function calculateIndicators(candles: CandleRow[], timestampMs: number): IndicatorSnapshot {
    ...
    const emaSlope = ema7Series.length > 8 && ema7 !== undefined
    ...
    const trendUp = ema7 !== undefined && ema25 !== undefined && ema99 !== undefined && ema7 > ema25 && ema25 > ema99 && (emaSlope ?? 0) > 0;
    const trendDown = ema7 !== undefined && ema25 !== undefined && ema99 !== undefined && ema7 < ema25 && ema25 < ema99 && (emaSlope ?? 0) < 0;
...
export function buildRegimeMetrics(evaluations: RegimeAuditEvaluation[]): RegimeGroupMetrics[] {
    ...
    return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([regime, rows]) => {
...
function buildRegimeMetricRow(regime: string, rows: RegimeAuditEvaluation[], side: Side | 'ALL' = 'ALL'): RegimeGroupMetrics {
    ...
    return {
        ...
        nextTwoCandlesFavorRate: booleanRate(rows.map(row => row.forwardChecks?.nextTwoCandlesClosedFavor)),
        avgFutureReturn60: round(avg(rows.map(row => row.outcomes['60m'].returnRoe).filter(isNumber))),
...
export function buildSymbolMetrics(evaluations: RegimeAuditEvaluation[]): SymbolGroupMetrics[] {
    ...
    return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([symbol, rows]) => {
        const byRegime = buildRegimeMetrics(rows).filter(row => row.count > 0);
        const ranked = [...byRegime].sort((a, b) => (b.avgFutureReturn60 ?? -Infinity) - (a.avgFutureReturn60 ?? -Infinity));
        const avgMfe60 = avg(rows.map(row => row.outcomes['60m'].mfeRoe).filter(isNumber));
        const avgMae60 = avg(rows.map(row => row.outcomes['60m'].maeRoe).filter(isNumber));
        return {
            symbol,
            mostCommonRegime: mostCommon(rows.map(row => row.regime)),
            countMomentum: rows.filter(row => row.regime === 'MOMENTUM_UP' || row.regime === 'MOMENTUM_DOWN').length,
            countChop: rows.filter(row => row.regime === 'CHOP').length,
            avgMfe60: round(avgMfe60),
            avgMae60: round(avgMae60),
            bestLabel: ranked[0]?.regime,
            worstLabel: ranked[ranked.length - 1]?.regime,
            comment: rows.length < 10 || avgMfe60 === undefined || avgMae60 === undefined
                ? 'low_sample'
                : avgMfe60 > Math.abs(avgMae60) ? 'favorable_mfe_mae' : 'noisy_or_adverse'
        };
    });
}

export function buildConfirmationSegments(evaluations: RegimeAuditEvaluation[]): ConfirmationSegmentMetrics[] {
    const segments: Array<{ name: string; rows: RegimeAuditEvaluation[] }> = [
        { name: 'momentum_pattern_only', rows: evaluations },
        { name: 'momentum_pattern + turbo_agreement', rows: evaluations.filter(row => row.turboAgreement) },
        { name: 'momentum_pattern + regime_confirm', rows: evaluations.filter(row => row.regimeConfirm) },
        {
        ...
        },
        {
            ...
            rows: evaluations.filter(row => row.turboAgreement && row.regimeConfirm && row.btcEthConfirm)
        },
        { name: 'momentum_pattern + BTC/ETH contradict', rows: evaluations.filter(row => row.btcEthContradict) }
    ];
    return segments.map(segment => confirmationMetrics(segment.name, segment.rows));
}

function confirmationMetrics(segment: string, rows: RegimeAuditEvaluation[]): ConfirmationSegmentMetrics {
    return {
        segment,
        count: rows.length,
...
function buildEvaluation(input: {
    ...
    const outcomes = Object.fromEntries(WINDOWS.map(minutes => [
        `${minutes}m`,
...
function loadCandles(dbPath: string, symbols: string[], timeframe: string, warnings: string[]): Map<string, CandleRow[]> {
...
function mergeMetadata(event?: AegisTurboTradeEventInput, open?: AegisTurboTradeOpenInput): MatchedMetadata {
    ...
    const cleanEntry = asRecord(asRecord(open?.metadata)?.cleanEntryGuard)
    ...
    return {
        ...
        entryQualityScore: numberOrNull(cleanEntry?.entryQualityModelScore ?? cleanEntry?.entryQualityScore),
        ...
        eventRiskWouldBlock: booleanOrUndefined(cleanEntry?.eventRiskWouldBlock ?? eventRisk?.wouldBlock),
        entryPolicyRegime: regimeGuard
    };
}

function nearestByTime<T extends { timestamp?: string }>(rows: T[], timestampMs: number, maxDistanceMs: number): T | undefined {
...
function buildTimedIndex<T extends { timestamp?: string }>(rows: T[], keyFn: (row: T) => string): Map<string, Timed<T>[]> {
...
function nearestTimed<T>(rows: Timed<T>[], timestampMs: number, maxDistanceMs: number): T | undefined {
    ...
    const candidates = [rows[insert - 1], rows[insert]].filter((row): row is Timed<T> => Boolean(row));
...
function buildSpecialCases(
    ...
    const cases: Array<{ label: string; close?: AegisTurboTradeCloseInput }> = [
        { label: 'ADA perdedora', close: latestTrade(closes, 'ADAUSDT', row => numberOrZero(row.net_pnl_usdt ?? row.pnl_usdt) < 0) },
        { label: 'AVAX ganadora', close: latestTrade(closes, 'AVAXUSDT', row => numberOrZero(row.net_pnl_usdt ?? row.pnl_usdt) > 0) },
        { label: 'ETH ganadora', close: latestTrade(closes, 'ETHUSDT', row => numberOrZero(row.net_pnl_usdt ?? row.pnl_usdt) > 0) },
        { label: 'LINK ganadora con MAE alto', close: highestMaeTrade(closes, 'LINKUSDT') },
        { label: 'SUI ganadora', close: latestTrade(closes, 'SUIUSDT', row => numberOrZero(row.net_pnl_usdt ?? row.pnl_usdt) > 0) }
    ];

    return cases.filter(item => item.close).map(item => {
        ...
        const nearestEval = openedAt
            ? nearestByTime(evaluations.filter(evaluation => evaluation.symbol === normalizeSymbol(close.symbol)), parseTimestamp(openedAt), 10 * 60_000)
            : undefined;
        const fallbackDecision = nearestEval ? undefined : evaluateSpecialCaseRegime(close, open, signalIndex, openedAtMs);
...
function latestTrade(
    ...
    return closes
        ...
        .sort((a, b) => parseTimestamp(b.closed_at ?? b.timestamp) - parseTimestamp(a.closed_at ?? a.timestamp))[0];
}

function highestMaeTrade(closes: AegisTurboTradeCloseInput[], symbol: string): AegisTurboTradeCloseInput | undefined {
    return closes
        .filter(row => normalizeSymbol(row.symbol) === symbol && numberOrZero(row.net_pnl_usdt ?? row.pnl_usdt) > 0)
        .sort((a, b) => Math.abs(numberOrZero(b.mae_roe)) - Math.abs(numberOrZero(a.mae_roe)))[0];
}

function indicatorWarning(regime: AegisRegimeLabel, indicators: IndicatorSnapshot): string | undefined {
    if (indicators.classification === 'UNKNOWN') return undefined;
    if (regime === 'MOMENTUM_UP' && (indicators.classification === 'CHOP' || indicators.classification === 'TREND_DOWN')) {
        return 'regime_momentum_up_indicators_chop_or_down';
    }
    if (regime === 'MOMENTUM_DOWN' && (indicators.classification === 'CHOP' || indicators.classification === 'TREND_UP')) {
        return 'regime_momentum_down_indicators_chop_or_up';
    }
    if (regime === 'CHOP' && (indicators.classification === 'TREND_UP' || indicators.classification === 'TREND_DOWN')) {
...
function btcEthConfirmsSide(side: Side, btcAction?: string, ethAction?: string): boolean {
    const expected = side;
    const actions = [normalizeActionText(btcAction), normalizeActionText(ethAction)].filter((value): value is Side => value === 'LONG' || value === 'SHORT');
    return actions.length > 0 && actions.every(action => action === expected);
}

function btcEthContradictsSideForActions(side: Side, btcAction?: string, ethAction?: string): boolean {
...
function calculateForwardChecks(candles: CandleRow[], timestampMs: number, side: Side): MomentumForwardChecks {
    const index = findEntryCandleIndex(candles, timestampMs);
    if (index < 0) return {};
    ...
    return {
        ...
        nextTwoCandlesClosedFavor: next && second
            ? candleClosedFavor(entry.close, next.close, side) && candleClosedFavor(next.close, second.close, side)
            : undefined,
...
function buildSpecialMomentumWindows(evaluations: RegimeAuditEvaluation[]): SpecialMomentumWindowAudit[] {
    const windows = [
        { label: 'XRPUSDT agosto 2025', symbol: 'XRPUSDT', from: '2025-08-01T00:00:00.000Z', to: '2025-08-31T23:59:59.999Z' },
...
async function writeAuditReports(report: RegimeAuditReport, reportsDir: string): Promise<RegimeAuditReport['outputFiles']> {
    ...
    await fs.writeFile(jsonlPath, `${report.evaluations.map(row => JSON.stringify(row)).join('\n')}\n`, 'utf8');
...
export function renderConsoleSummary(report: RegimeAuditReport): string {
    ...
    lines.push(`Mode: ${report.options.momentumOnly ? 'momentum-only offline candidates' : 'turbo-signal audit'}`);
    lines.push(`Evaluations: ${report.counts.evaluations} | Signals loaded: ${report.counts.signalsLoaded} | Skipped: ${report.counts.skippedSignals}`);
    if (report.warnings.length > 0) lines.push(`Warnings: ${report.warnings.join('; ')}`);
    lines.push('');
    lines.push('By regime');
    lines.push(formatTable(report.byRegime.map(row => ({
        regime: row.regime,
        count: row.count,
    ...
    lines.push(formatTable(report.byRegimeSide.map(row => ({
        side: row.side,
        regime: row.regime,
        count: row.count,
        avgMfe60: row.avgMfe60,
    ...
    lines.push(formatTable(report.bySymbol.map(row => ({
        symbol: row.symbol,
    ...
    lines.push(formatTable(report.confirmationSegments.map(row => ({
        segment: row.segment,
        count: row.count,
        avgMfe60: row.avgMfe60,
        avgMae60: row.avgMae60,
    ...
    if (report.specialCases.length > 0) {
        ...
        lines.push(formatTable(report.specialCases.map(row => ({
            label: row.label,
            symbol: row.symbol,
            side: row.side,
            pnl: row.pnlUsdt,
    ...
    if (report.specialMomentumWindows.length > 0) {
        ...
        lines.push(formatTable(report.specialMomentumWindows.map(row => ({
            label: row.label,
            symbol: row.symbol,
...
function renderMomentumRecommendation(report: RegimeAuditReport): string {
    ...
    const turboRegime = report.confirmationSegments.find(row => row.segment === 'momentum_pattern + turbo_agreement + regime_confirm');
    const btcEth = report.confirmationSegments.find(row => row.segment === 'momentum_pattern + turbo_agreement + regime_confirm + BTC/ETH confirm');
    return [
    ...
    return rows
        .filter(row => {
            ...
            return allow
                ...
                : avgRet < 0 || hit8 < 0.40 || reversal > 0.60;
        })
        .sort((a, b) => (b.avgFutureReturn60 ?? -Infinity) - (a.avgFutureReturn60 ?? -Infinity))
        .map(row => row.regime);
}
...
function renderMarkdown(report: RegimeAuditReport): string {
    ...
    lines.push(markdownTable(report.inconsistencies.slice(0, 25).map(row => ({
...
function calculateChoppiness(candles: CandleRow[]): number | undefined {
    ...
    const range = max(candles.map(candle => candle.high)) - min(candles.map(candle => candle.low));
...
function resolveSide(signal: AegisTurboSignalHistoryInput, includeHold: boolean): Side | undefined {
    ...
    if (includeHold && (signal.raw_action === 'LONG' || signal.raw_action === 'SHORT')) return signal.raw_action;
    return undefined;
}

function actionOf(signal?: AegisTurboSignalHistoryInput): string | undefined {
    if (!signal) return undefined;
    return stringOrUndefined(signal.final_action) ?? stringOrUndefined(signal.gated_action) ?? stringOrUndefined(signal.raw_action);
}
...
function parseTimestamp(value: unknown): number {
    ...
    if (!text) return NaN;
    const parsed = Date.parse(text.endsWith('Z') || /[+-]\d{2}:?\d{2}$/.test(text) ? text : `${text}Z`);
    return parsed;
}
...
function normalizeVotes(value: unknown): AegisTurboVotes | undefined {
    ...
    return votes.long === undefined && votes.short === undefined && votes.neutral === undefined ? undefined : votes;
}
...
function classifyRegime(count: number, avgMfe?: number, avgMae?: number, hit5?: number): RegimeGroupMetrics['conclusion'] {
