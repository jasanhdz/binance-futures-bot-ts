...
export async function exportRegimeEngineV2MlDataset(
  options: RegimeEngineV2MlDatasetOptions = {},
): Promise<RegimeEngineV2MlDatasetResult> {
  ...
  result.outputFiles = await writeDatasetFiles(
    result,
    options.outputDir ?? options.reportsDir ?? '/home/jasan/Develop',
  );
  return result;
}
...
export function buildRegimeEngineV2MlDatasetRows(
  ...
  const { samples, sampleEvery, leverage, feeBps, slippageBps } = buildRegimeEngineV2AuditSamples(
    candlesBySymbol,
    {
      ...options,
      momentumPatternOnly: true,
...
function datasetRow(
  ...
  return {
    ...
    target_false_breakout:
      outcome.falseBreakout === undefined ? undefined : outcome.falseBreakout ? 1 : 0,
...
async function writeDatasetFiles(
  result: RegimeEngineV2MlDatasetResult,
  outputDir: string,
): Promise<RegimeEngineV2MlDatasetResult['outputFiles']> {
