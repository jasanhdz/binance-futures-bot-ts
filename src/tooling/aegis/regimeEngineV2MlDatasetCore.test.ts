...
describe('regimeEngineV2MlDatasetCore', () => {
  it('exports expected columns for momentum pattern samples', () => {
    const rows = buildRegimeEngineV2MlDatasetRows(
      new Map<string, RegimeEngineV2InputCandle[]>([
  ...
  it('keeps future targets out of feature-like columns', () => {
    const rows = buildRegimeEngineV2MlDatasetRows(
      new Map<string, RegimeEngineV2InputCandle[]>([['BTCUSDT', patternCandles('LONG', 160)]]),
      {
        symbols: ['BTCUSDT'],
        sampleEvery: 6,
      },
  ...
  it('supports legacy XRP long pattern dataset rows', () => {
    const rows = buildRegimeEngineV2MlDatasetRows(
      new Map<string, RegimeEngineV2InputCandle[]>([['XRPUSDT', legacyXrpLongCandles(180)]]),
      {
        symbols: ['XRPUSDT'],
        sampleEvery: 3,
