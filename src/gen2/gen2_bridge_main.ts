...
import { emergencyDenyActive, readConfigExecutionEnabled, readConfigSymbols, resolveConfigPath } from './gen2BridgeConfig';
...
function main(): void {
  ...
  // SINGLE SOURCE OF TRUTH: config enables execution AND defines the executable
  // symbols (env can only deny execution, never expand/replace the symbol list).
  const configPath = resolveConfigPath(repoRoot, process.env.GEN2_CONFIG_PATH);
  const cfgExec = readConfigExecutionEnabled(configPath);
  const allowedSymbols = readConfigSymbols(configPath);
