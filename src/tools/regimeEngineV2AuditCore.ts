...
export type RegimeEngineV2AuditReport = {
    generatedAt: string;
    options: Required<Pick<RegimeEngineV2AuditOptions, 'sampleEvery' | 'leverage' | 'timeframe'>> & RegimeEngineV2AuditOptions;
    counts: {
...
export async function auditRegimeEngineV2(options: RegimeEngineV2AuditOptions = {}): Promise<RegimeEngineV2AuditReport> {
...
function loadCandles(dbPath: string, symbols: string[], timeframe: string, warnings: string[], fromMs: number, toMs: number): Map<string, RegimeEngineV2InputCandle[]> {
    ...
    try {
        ...
        for (const symbol of symbols) {
            ...
            const candles = rows
                .map(toCandle)
                .filter((candle) => (candle.timestamp ?? 0) >= fromMs - 2 * 24 * 60 * 60 * 1000 && (candle.timestamp ?? 0) <= toMs + 2 * 60 * 60 * 1000);
...
function marketContext(symbol: string, index: number, decisionFor: (symbol: string, index: number) => RegimeEngineV2Decision | undefined): RegimeEngineV2MarketContext | undefined {
    ...
    return {
        btc: btc ? { action: actionFromEnv(btc.momentumEnvironment), score: btc.confidence, direction: btc.technicalDirection } : undefined,
        eth: symbol === 'ETHUSDT' ? undefined : eth ? { action: actionFromEnv(eth.momentumEnvironment), score: eth.confidence, direction: eth.technicalDirection } : undefined
    };
}
...
export function directionalEnvironmentBucket(environment: RegimeEngineV2MomentumEnvironment, side: Side): RegimeEngineV2DirectionalEnvironment {
    if (side === 'LONG') {
...
export function detectMomentumRidePattern(candles: RegimeEngineV2InputCandle[], index: number): RegimeEngineV2MomentumPattern | undefined {
    ...
    const longConsecutive = last3.every((candle) => candle.close > candle.open)
        && last3[2].close > last3[1].close
        && last3[1].close > last3[0].close;
    const shortConsecutive = last3.every((candle) => candle.close < candle.open)
        && last3[2].close < last3[1].close
        && last3[1].close < last3[0].close;
    ...
    if ((longConsecutive || breakoutUp) && hasVolume && closeLocation >= 0.62 && upperWickRatio <= 0.42 && notOverextended) {
        return {
            side: 'LONG',
            kind: longConsecutive && breakoutUp ? 'CONSECUTIVE_BREAKOUT' : longConsecutive ? 'CONSECUTIVE' : 'BREAKOUT',
            reasons: [
    ...
    if ((shortConsecutive || breakoutDown) && hasVolume && closeLocation <= 0.38 && lowerWickRatio <= 0.42 && notOverextended) {
        return {
            side: 'SHORT',
            kind: shortConsecutive && breakoutDown ? 'CONSECUTIVE_BREAKOUT' : shortConsecutive ? 'CONSECUTIVE' : 'BREAKOUT',
            reasons: [
...
export function calculateRegimeEngineV2Outcome(
    ...
    return {
        ...
        hit10BeforeMinus8: hitThreshold(future, entry.close, side, leverage, 0.10, 0.08, totalCostRoe),
        ...
        timeTo10Minutes: timeToTarget(future, entry.close, side, leverage, 0.10, totalCostRoe),
        falseBreakout: decision.technicalRegime.includes('BREAKOUT') ? netForwardReturnRoe < 0 || netMfeRoe < 0.05 : undefined,
        postSignalDrawdownRoe: round(netMaeRoe),
        totalCostRoe: round(totalCostRoe)
    };
}

function hitThreshold(candles: RegimeEngineV2InputCandle[], entryPrice: number, side: Side, leverage: number, target: number, adverse: number, costRoe = 0): HitOutcome {
...
function timeToTarget(candles: RegimeEngineV2InputCandle[], entryPrice: number, side: Side, leverage: number, target: number, costRoe = 0): number | undefined {
...
function metricRow(bucket: string, samples: RegimeEngineV2AuditSample[], horizon: '15m' | '30m' | '60m' | '120m'): RegimeEngineV2MetricRow {
    ...
    return {
        ...
        avgForwardReturnRoe: round(avg(outcomes.map((outcome) => outcome.forwardReturnRoe).filter(isNumber))),
        ...
        p90MaeRoe: round(percentile(outcomes.map((outcome) => Math.abs(outcome.maeRoe ?? 0)).filter(isNumber), 0.9)),
...
function buildWalkForward(samples: RegimeEngineV2AuditSample[]): RegimeEngineV2WalkForwardRow[] {
    ...
    const testDays = 5;
    const stepDays = 5;
    for (let trainFrom = start; trainFrom + (trainDays + testDays) * dayMs() <= end; trainFrom += stepDays * dayMs()) {
        const trainTo = trainFrom + trainDays * dayMs();
        const testFrom = trainTo;
        const testTo = testFrom + testDays * dayMs();
        const train = samples.filter((sample) => sample.timestampMs >= trainFrom && sample.timestampMs < trainTo);
        const test = samples.filter((sample) => sample.timestampMs >= testFrom && sample.timestampMs < testTo);
        for (const bucket of directionalEnvironmentBuckets()) {
            ...
            rows.push({
                ...
                trainHit8,
                testHit8,
                decay: trainHit8 !== undefined && testHit8 !== undefined ? round(testHit8 - trainHit8) : undefined,
                label: sufficient ? (testHit8 ?? 0) >= (trainHit8 ?? 0) - 0.08 ? 'stable' : 'decay' : 'insufficient'
            });
        }
    }
    return rows;
...
function buildEnvironmentDiagnostics(samples: RegimeEngineV2AuditSample[]): RegimeEngineV2EnvironmentDiagnosticRow[] {
    return directionalEnvironmentBuckets()
        .map((bucket) => {
            ...
            return {
                ...
                avgTransitionRiskScore: round(avg(group.map((sample) => transitionRiskScore(sample.decision.transition.risk)))),
                avgMarketConfirmationScore: round(avg(group.map((sample) => sample.decision.scores.marketConfirmationScore))),
                p90MaeRoe: round(percentile(outcomes60.map((outcome) => Math.abs(outcome.maeRoe ?? 0)).filter(isNumber), 0.9)),
...
function buildRecommendations(report: RegimeEngineV2AuditReport): string[] {
    ...
    const early = report.byEarlyMatureExhausted.find((row) => row.bucket === 'EARLY' && row.horizon === '60m');
    const mature = report.byEarlyMatureExhausted.find((row) => row.bucket === 'MATURE' && row.horizon === '60m');
    const allowShortDiag = report.environmentDiagnostics.find((row) => row.bucket === 'ALLOW_SHORT_MOMENTUM');
    const watchShortDiag = report.environmentDiagnostics.find((row) => row.bucket === 'WATCH_SHORT_MOMENTUM');
    return [
...
function walkForwardTable(rows: RegimeEngineV2WalkForwardRow[]): string {
    ...
    return [
        ...
        '|---|---:|---:|---:|',
        ...[...summary.entries()].map(([bucket, counts]) => `| ${bucket} | ${counts.stable} | ${counts.decay} | ${counts.insufficient} |`)
    ].join('\n');
}
...
function metricsTable(rows: RegimeEngineV2MetricRow[]): string {
    const filtered = rows.filter((row) => row.horizon === '60m').sort((a, b) => b.count - a.count || a.bucket.localeCompare(b.bucket));
    if (filtered.length === 0) return '_No rows._';
    return [
        '| bucket | count | avgMFE | avgMAE | MFE/MAE | hit5 | hit8 | hit10 | p90 MAE | avg t+8 | false breakout |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
        ...filtered.map((row) => `| ${row.bucket} | ${row.count} | ${fmt(row.avgMfeRoe)} | ${fmt(row.avgMaeRoe)} | ${fmt(row.mfeMaeRatio)} | ${fmt(row.hit5BeforeMinus5Rate)} | ${fmt(row.hit8BeforeMinus5Rate)} | ${fmt(row.hit10BeforeMinus8Rate)} | ${fmt(row.p90MaeRoe)} | ${fmt(row.avgTimeTo8Minutes)} | ${fmt(row.falseBreakoutRate)} |`)
    ].join('\n');
}
...
function environmentDiagnosticsTable(rows: RegimeEngineV2EnvironmentDiagnosticRow[]): string {
    ...
    return [
        ...
        '|---|---:|---|---|---:|---:|---:|---:|',
        ...rows.map((row) => `| ${row.bucket} | ${row.count} | ${row.topTechnicalRegimes || 'n/a'} | ${row.topSymbols || 'n/a'} | ${fmt(row.avgTransitionRiskScore)} | ${fmt(row.avgMarketConfirmationScore)} | ${fmt(row.p90MaeRoe)} | ${fmt(row.falseBreakoutRate)} |`)
    ].join('\n');
}
