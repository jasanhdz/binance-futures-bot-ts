...
describe('longRiskShadowAuditCore', () => {
    it('summarizes high-risk LONG warnings without counting SHORT controls', async () => {
        ...
        await writeFile(path.join(logsDir, 'turbo_trades_2026-05-22.jsonl'), rows.map((row) => JSON.stringify(row)).join('\n'));
...
function openTrade(tradeId: string, symbol: string, side: 'LONG' | 'SHORT', pnl: number, entryPolicy: Record<string, unknown>) {
    return {
        trade_id: tradeId,
        status: 'OPEN',
        symbol,
