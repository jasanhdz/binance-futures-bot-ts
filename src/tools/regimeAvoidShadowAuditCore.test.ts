...
describe('auditRegimeAvoidShadow', () => {
    ...
    it('uses ENTRY_POLICY_DECISION metadata when open trade metadata is missing', async () => {
        ...
        await writeEvents([
            entryPolicyEvent('t1', 'ADAUSDT', 'LONG', 'CHOP')
        ]);
    ...
    it('handles corrupted logs and skips duplicate closes', async () => {
        ...
        await fs.writeFile(path.join(logsDir, 'turbo_trades_2026-05-22.jsonl'), `${rows.join('\n')}\n`, 'utf8');
...
function closeTrade(tradeId: string, symbol: string, side: 'LONG' | 'SHORT', pnl: number, roe: number) {
    return {
