...
export async function runE4EntryMaeStudy(options: E4EntryMaeStudyOptions = {}): Promise<JsonRecord> {
    ...
    await streamJsonlFiles(logsDir, tradeFiles, warnings, (row) => {
        ...
        if (!tradeId || !Number.isFinite(timestampMs) || timestampMs < fromMs || timestampMs > toMs) return;
    ...
    await streamJsonlFiles(logsDir, eventFiles, warnings, (row) => {
        ...
        if (Number.isFinite(timestampMs) && roe !== undefined) item.observations.push({ timestamp, timestampMs, roe });
    });
    ...
    for (const [tradeId, pair] of pairs) {
        ...
        const input = toStudyInput(tradeId, pair.open, pair.close, item.e4Event, item.observations, warnings);
...
export function analyzeTradePath(input: TradePathStudyInput) {
    ...
    const classification = first30MaeRoe !== undefined && (first30MaeRoe <= -0.05 || Boolean(minus5 && (!plus5 || minus5.timestampMs < plus5.timestampMs)))
        ? 'EARLY_ENTRY_SUFFERING'
        : first30MaeRoe !== undefined && first30MaeRoe > -0.05 && input.recordedMaeRoe <= -0.10 && plus5BeforeMinus5
            ? 'LATE_REVERSAL'
            : 'CLEAN_PATH';
    ...
    return {
        ...
        protectionCounterfactuals: PROTECTION_POLICIES.map((proposal) => simulateProtection(input, proposal)),
        leverageStress: STRESS_LEVERAGES.map((leverage) => ({
            leverage,
            estimatedMaeRoe: input.recordedMaeRoe * leverage / input.leverage,
            breachesConfiguredStop: input.recordedMaeRoe * leverage / input.leverage <= -0.40
        }))
    };
}
...
export function simulateDelayedEntry(input: TradePathStudyInput, delayMinutes: number) {
    ...
    const futureRoes = input.observations
        ...
        .map((row) => roeFromPrice(markFromRoe(input.entryPrice, input.side, input.leverage, row.roe), delayedEntryPrice, input.side, input.leverage));
...
export function simulateProtection(input: TradePathStudyInput, policy: { name: string; triggerRoe: number; floorRoe: number }) {
    const trigger = input.observations.find((row) => row.roe >= policy.triggerRoe);
    if (!trigger) return { ...policy, triggered: false, exited: false, simulatedFinalRoe: input.finalRoe, finalRoeDelta: 0, simulatedMaeRoe: input.recordedMaeRoe, maeImprovement: 0 };
    const exit = input.observations.find((row) => row.timestampMs > trigger.timestampMs && row.roe <= policy.floorRoe);
    ...
    const observedPath = exit
        ...
        : input.observations;
    const simulatedMaeRoe = exit ? minimum(observedPath.map((row) => row.roe)) ?? input.recordedMaeRoe : input.recordedMaeRoe;
    return {
        ...policy,
        triggered: true,
...
export function renderE4EntryMaeStudyMarkdown(report: JsonRecord): string {
    ...
    for (const trade of report.trades) {
        lines.push(`| ${trade.symbol} | ${trade.side} | ${pct(trade.finalRoe)} | ${pct(trade.first30MaeRoe)} | ${pct(trade.recordedMaeRoe)} | ${num(trade.minutesToPlus5)} | ${num(trade.minutesToMinus5)} | ${trade.classification} | ${num(trade.entryEvidence.e4Score)} | ${num(trade.entryEvidence.tailRiskScore)} |`);
    }
    lines.push('', '## Delay Policies', '', '| Delay | N | Mean MAE | Mean final ROE | Mean delta | Improved final |', '|---:|---:|---:|---:|---:|---:|');
    for (const row of report.delayPolicies) {
        lines.push(`| ${row.delayMinutes}m | ${row.count} | ${pct(row.meanMaeRoe)} | ${pct(row.meanFinalRoe)} | ${pct(row.meanFinalRoeDelta)} | ${row.improvedFinalCount} |`);
    }
    lines.push('', '## Protection Policies', '', '| Policy | Triggered | Exited | Mean actual | Mean simulated | Mean delta | Mean MAE | MAE improvement |', '|---|---:|---:|---:|---:|---:|---:|---:|');
    for (const row of report.protectionPolicies) {
        lines.push(`| ${row.name} | ${row.triggeredCount} | ${row.exitedCount} | ${pct(row.meanActualFinalRoe)} | ${pct(row.meanSimulatedFinalRoe)} | ${pct(row.meanFinalRoeDelta)} | ${pct(row.meanSimulatedMaeRoe)} | ${pct(row.meanMaeImprovement)} |`);
    }
    lines.push('', '## Evidence-backed Proposals', '');
    for (const proposal of report.proposals) lines.push(`- **${proposal.status} ${proposal.name}:** ${proposal.evidence}`);
    lines.push('', '## Interpretation Rules', '', '- EARLY_ENTRY_SUFFERING: investigate confirmation or pullback/reclaim.', '- LATE_REVERSAL: investigate earlier profit protection, not delayed entry.', '- CLEAN_PATH: use as a control; avoid filters that remove these trades.', '', '## Limitations', '');
...
function buildSummary(trades: TradePathStudy[]) {
    ...
    return {
        ...
        classifications: Object.fromEntries(['EARLY_ENTRY_SUFFERING', 'LATE_REVERSAL', 'CLEAN_PATH'].map((name) => [name, trades.filter((trade) => trade.classification === name).length]))
    };
}

function aggregateDelayPolicies(trades: TradePathStudy[]) {
    return DELAY_MINUTES.map((delayMinutes) => {
        const rows = trades.map((trade) => trade.delayCounterfactuals.find((row) => row.delayMinutes === delayMinutes)).filter((row) => row?.available) as JsonRecord[];
        return {
            delayMinutes,
            count: rows.length,
...
function aggregateProtectionPolicies(trades: TradePathStudy[]) {
    return PROTECTION_POLICIES.map((policy) => {
        const rows = trades.map((trade) => trade.protectionCounterfactuals.find((row) => row.name === policy.name)!).filter(Boolean);
        return {
            name: policy.name,
...
function buildProposals(trades: TradePathStudy[], delayPolicies: JsonRecord[], protectionPolicies: JsonRecord[]) {
    const bestProtection = protectionPolicies.find((row) => row.name === 'trigger_8_floor_5');
    const avax = trades.find((trade) => trade.symbol === 'AVAXUSDT' && trade.recordedMaeRoe < -0.20);
    const dirtyWinners = trades.filter((trade) => trade.finalRoe > 0 && trade.entryEvidence.dirty).length;
    const thirtyXBreaches = trades.filter((trade) => trade.leverageStress.some((row) => row.leverage === 30 && row.breachesConfiguredStop)).length;
    const delayEvidence = delayPolicies.map((row) => `${row.delayMinutes}m ${pct(row.meanFinalRoeDelta)}`).join(', ');
    return [
        {
...
function toStudyInput(tradeId: string, open: JsonRecord, close: JsonRecord, e4Event: JsonRecord, observations: RoeObservation[], warnings: string[]): TradePathStudyInput | undefined {
...
async function streamJsonlFiles(logsDir: string, files: string[], warnings: string[], consume: (row: JsonRecord) => void): Promise<void> {
...
function selectDatedLogFiles(files: string[], prefix: string, fromMs: number, toMs: number): string[] {
...
function markFromRoe(entryPrice: number, side: 'LONG' | 'SHORT', leverage: number, roe: number): number {
    return side === 'LONG' ? entryPrice * (1 + roe / leverage) : entryPrice * (1 - roe / leverage);
}

function roeFromPrice(price: number, entryPrice: number, side: 'LONG' | 'SHORT', leverage: number): number {
    return side === 'LONG' ? (price / entryPrice - 1) * leverage : (1 - price / entryPrice) * leverage;
}

function minBy<T>(rows: T[], value: (row: T) => number): T | undefined {
    return rows.reduce<T | undefined>((best, row) => best === undefined || value(row) < value(best) ? row : best, undefined);
}
