...
export interface LongRiskShadowAuditReport {
    generatedAt: string;
    options: Required<Pick<LongRiskShadowAuditOptions, 'repoRoot' | 'logsDir' | 'outDir'>> & LongRiskShadowAuditOptions;
...
export async function auditLongRiskShadow(options: LongRiskShadowAuditOptions = {}): Promise<LongRiskShadowAuditReport> {
    ...
    const rows = await loadJsonlByPrefix(logsDir, 'turbo_trades', dateKeysBetween(fromMs, toMs), warnings);
    ...
    const trades = Array.from(pairs.values())
        .filter((pair) => {
            ...
            return pair.symbol && pair.open && pair.close && openedMs >= fromMs && openedMs <= toMs && (pair.open.side ?? pair.close.side) === 'LONG';
        })
...
function buildTradeAudit(pair: TradePair): LongRiskShadowTradeAudit {
    ...
    const assessment = evaluateAegisLongRiskShadow({
        ...
        entryQualityRecommendation: guards.entry_quality?.metadata?.recommendation ?? entryPolicy.entryQualityRecommendation,
        entryQualityScore: num(guards.entry_quality?.metadata?.entryQualityScore ?? guards.decision_brain?.metadata?.entryQualityModelScore),
        tailRiskScore: num(guards.entry_quality?.metadata?.tailRiskScore ?? guards.decision_brain?.metadata?.tailRiskScore),
        cleanEntryDecision: guards.clean_entry?.decision,
        eventRiskMode: guards.clean_entry?.metadata?.eventRiskMode ?? guards.event_risk?.mode ?? guards.event_risk?.metadata?.mode,
        eventRiskWouldBlock: guards.event_risk?.wouldBlock ?? guards.clean_entry?.metadata?.eventRiskWouldBlock,
...
function groupedSummary(trades: LongRiskShadowTradeAudit[], keyFn: (trade: LongRiskShadowTradeAudit) => string): Record<string, JsonRecord> {
...
async function writeReports(report: LongRiskShadowAuditReport, outDir: string): Promise<NonNullable<LongRiskShadowAuditReport['outputFiles']>> {
