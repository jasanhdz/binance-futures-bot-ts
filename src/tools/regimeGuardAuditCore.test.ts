...
function candle(minute: number, open: number, high: number, low: number, close: number, volume = 100): CandleRow {
...
describe('regimeGuardAuditCore', () => {
    ...
    it('detects target threshold before adverse threshold', () => {
        const future = [
            candle(5, 100, 100.3, 99.9, 100.2),
            candle(10, 100.2, 100.5, 99.7, 100.4)
        ];
    ...
    it('detects adverse threshold before target threshold', () => {
        const future = [
            candle(5, 100, 100.1, 99.4, 99.8),
            candle(10, 99.8, 100.7, 99.7, 100.5)
        ];
    ...
    it('groups regime metrics with hit rates and conclusions', () => {
        const rows = [
            evaluation('ETHUSDT', 'MOMENTUM_UP', 0.30, -0.10, 'TARGET_FIRST'),
            evaluation('BTCUSDT', 'MOMENTUM_UP', 0.20, -0.10, 'TARGET_FIRST'),
            evaluation('ADAUSDT', 'CHOP', 0.05, -0.20, 'ADVERSE_FIRST')
        ];
    ...
    it('groups symbol metrics and picks best/worst labels', () => {
        const rows = [
            evaluation('ETHUSDT', 'MOMENTUM_UP', 0.30, -0.10, 'TARGET_FIRST'),
            evaluation('ETHUSDT', 'CHOP', 0.05, -0.20, 'ADVERSE_FIRST'),
            evaluation('BTCUSDT', 'MOMENTUM_DOWN', 0.20, -0.08, 'TARGET_FIRST')
        ];
    ...
    it('builds regime side and confirmation segment metrics', () => {
        ...
        expect(bySide.find(row => row.side === 'LONG' && row.regime === 'MOMENTUM_UP')?.count).toBe(1);
