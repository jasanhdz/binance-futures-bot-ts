...
function confidenceForOpposite(input: AegisExitEyeInput): AegisExitEyeConfidence {
  ...
  if (oppositeVotes > input.oppositeVotesToClose || finite(input.turboScore) >= 0.7) return 'high';
  return 'medium';
}
...
export function evaluateAegisExitEye(input: AegisExitEyeInput): AegisExitEyeDecision {
  ...
  if (
    typeof input.minMinutesInTrade === 'number' &&
    finite(input.minutesInTrade) < input.minMinutesInTrade
  ) {
  ...
  const oppositeReversal =
    ...
    (currentTurboAction === oppositeAction ||
  ...
  if (neutralDecay) {
    ...
    const minPeakRoeToCloseOnNeutral =
      input.minPeakRoeToCloseOnNeutral ?? input.minPeakRoeToProtect;
    const minGivebackToCloseOnNeutral =
      input.minGivebackToCloseOnNeutral ?? input.minGivebackFromPeakRoe;
