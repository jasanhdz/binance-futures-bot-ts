...
export type AegisEntryQualityGateAction = 'ALLOW' | 'SHADOW_ALLOW' | 'SHADOW_BLOCK' | 'BLOCK';
...
function computeRecentReturn(
  candles: AegisEntryQualityGateCandle[],
  lookback: number,
): number | undefined {
...
export function evaluateAegisEntryQualityGate(
  ...
  const emaDistance =
    finiteNumber(input.currentPrice) && finiteNumber(input.emaFast) && input.emaFast > 0
      ? (input.currentPrice - input.emaFast) / input.emaFast
      : undefined;

  const metadata = baseMetadata({ ...input, symbol, mode }, { recentReturn, emaDistance });
