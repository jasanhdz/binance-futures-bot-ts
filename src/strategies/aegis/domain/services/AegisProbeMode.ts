...
function entriesLastHour(input: AegisProbeModeInput): number {
  ...
  return (input.probeEntryTimestamps ?? []).filter(
    (ts) => finiteNumber(ts) && ts >= cutoff && ts <= input.nowMs,
  ).length;
}

function metadata(
  input: AegisProbeModeInput,
  allowed: boolean,
  reason: AegisProbeModeReason,
): AegisProbeModeDecision['metadata'] {
  return {
...
function allow(input: AegisProbeModeInput): AegisProbeModeDecision {
  return {
    ...
    metadata: metadata(input, true, 'probe_allowed'),
  };
}
...
function hasEntryQualityBlock(input: AegisProbeModeInput): boolean {
  return (
    normalized(input.entryQualityRecommendation) === 'BLOCK_SHADOW' ||
...
export class AegisProbeMode {
  static evaluate(input: AegisProbeModeInput): AegisProbeModeDecision {
    ...
    if (decisionBrain !== requiredDecision)
      return deny(input, 'probe_decision_brain_not_enter_now');
    if (hasEntryQualityBlock(input)) return deny(input, 'probe_entry_quality_block_shadow');
    if (
      config.require_entry_quality_allow &&
      !isAllowedEntryQuality(input.entryQualityRecommendation)
    ) {
    ...
    if (
      !finiteNumber(input.featureParityPct) ||
      input.featureParityPct < config.min_feature_parity_pct
    ) {
