  ...
  static evaluate(input: AegisLongRiskShadowGuardAdapterInput): AegisEntryGuardResult {
    ...
    return {
      ...
      metadata: {
        ...
        actionTaken: 'SHADOW',
        shadow:
          isGuardShadow(input.policy) ||
          input.policy.mode === 'ENFORCE' ||
          input.policy.mode === 'ENFORCE_PROBE_LONG_CRITICAL',
      },
    };
  }
}

export function evaluateAegisLongRiskShadow(
  ...
  if (
    entryQualityReason.includes('insufficient_data') ||
  ...
  if (
    environment === 'AVOID_MOMENTUM' ||
    environment === 'WATCH_SHORT_MOMENTUM' ||
    environment === 'ALLOW_SHORT_MOMENTUM'
  ) {
    score += 0.3;
    reasons.push('long_risk_shadow_weak_regime');
  }
  ...
  if (
    technicalRegime.includes('MOMENTUM_DOWN') ||
    technicalRegime.includes('TREND_DOWN') ||
    technicalRegime.includes('EXHAUSTED') ||
    technicalRegime.includes('CHOP')
  ) {
    score += 0.2;
  ...
  if (isFiniteNumber(market.closeLocation) && market.closeLocation < 0.3) {
    score += 0.1;
    reasons.push('long_risk_shadow_down_momentum');
  }
  ...
  const decision = riskScore >= 0.3 ? 'SHADOW_DENY' : 'ALLOW';
  const reason =
    decision === 'ALLOW'
      ? 'long_risk_shadow_clear'
      : uniqueReasons.length > 1
        ? 'long_risk_shadow_multi_factor'
        : (uniqueReasons[0] ?? 'long_risk_shadow_multi_factor');
...
function buildInput(input: AegisLongRiskShadowGuardAdapterInput): AegisLongRiskShadowInput {
  ...
  return {
    ...
    entryQualityRecommendation:
      context.entryQuality.recommendation ?? context.entryQuality.model?.recommendation,
    entryQualityScore:
      context.entryQuality.entryQualityScore ?? context.entryQuality.model?.entry_quality_score,
    tailRiskScore:
      context.entryQuality.tailRiskScore ?? context.entryQuality.model?.tail_risk_score,
...
function marketWeaknessFromContext(context: AegisEntryContext): AegisLongRiskShadowMarketWeakness {
  ...
  return {
    belowEma25:
      isFiniteNumber(currentPrice) && isFiniteNumber(ema25) ? currentPrice < ema25 : undefined,
    return30m:
      isFiniteNumber(last?.close) && isFiniteNumber(previous6?.close)
        ? (last.close - previous6.close) / previous6.close
        : undefined,
    return60m:
      isFiniteNumber(last?.close) && isFiniteNumber(previous12?.close)
        ? (last.close - previous12.close) / previous12.close
        : undefined,
    closeLocation: last && range && range > 0 ? (last.close - last.low) / range : undefined,
    volumeRatio:
      isFiniteNumber(last?.volume) && avgVolume20 ? last.volume / avgVolume20 : undefined,
  };
}
...
function suggestedActionFor(
  level: AegisLongRiskShadowRiskLevel,
): AegisLongRiskShadowSuggestedAction {
