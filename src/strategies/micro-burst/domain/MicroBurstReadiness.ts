...
export interface MicroBurstReadinessInput {
  ...
  symbolBlockers?: Record<string, string[]>;
}

export interface MicroBurstReadinessResult {
  ready: boolean;
  readyForSoak: boolean;
...
export function assessMicroBurstReadiness(
  input: MicroBurstReadinessInput,
): MicroBurstReadinessResult {
  ...
  const checks: MicroBurstReadinessChecks = {
    ...
    storage:
      input.storageHealthy === true && (input.storageErrors ?? Number.MAX_SAFE_INTEGER) === 0,
    gaps: input.unresolvedTradeGaps !== undefined && input.unresolvedTradeGaps === 0,
    books:
      input.healthyBookCount !== undefined &&
      input.healthyBookCount === input.enabledSymbolCount &&
      input.enabledSymbolCount > 0,
  ...
  const soakBlockers = Object.entries(checks)
    .filter(([, value]) => !value)
    .map(([name]) => `${name.toUpperCase()}_NOT_READY`);
  ...
  const officialAuthority = readyForFreeze;
  const blockers = [...soakBlockers];
  return {
    ready: readyForSoak,
    readyForSoak,
    readyForFreeze,
    ...
    blockers,
    warnings,
    symbolBlockers: input.symbolBlockers ?? {},
  };
}
